...
defaultCommittedRecordMetadata o =
  CommittedRecordMetadata
    { crmOffset = o
...
data ProducerError
  = -- | ms exceeded
    PEDeliveryTimeout !Int
  | PEAccumulatorClosed
  | PEAccumulatorFull
  | -- | raw error code + broker message
    PEBrokerError !Int16 !Text
  | PERequestFailed !Text
  | PEFenced !Text
  | PEAuthorizationFailed !Text
  | -- | size in bytes that exceeded the cap
    PERecordTooLarge !Int
  | -- | KIP-1218
    PECorruptRecord !Text
  | PEUnknown !Text
  deriving stock (Eq, Show, Generic)
