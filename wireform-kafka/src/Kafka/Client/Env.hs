...
-}
data EnvAcks
  = -- | acks=0 (fire-and-forget; 'AtMostOnce')
    EnvAcksZero
  | -- | acks=1 (leader ACK; 'AtLeastOnce')
    EnvAcksOne
  | -- | acks=-1\/all (all ISRs; 'ExactlyOnce')
    EnvAcksAll
  deriving (Eq, Show)
...
fieldParsers =
  ...
  ,
    ( "KAFKA_SOCKET_KEEPALIVE_ENABLE"
    , boolField
        "socket.keepalive.enable"
        (\b env -> env {envSocketKeepaliveEnable = Just b})
    )
  ,
    ( "KAFKA_SOCKET_NAGLE_DISABLE"
    , boolField
...
parseCompression t = case Compression.parseCompressionCodec t of
  ...
  Nothing ->
    Left
      ( "expected one of none/gzip/snappy/lz4/zstd, got "
          ++ show (T.unpack t)
      )
...
buildSaslRequired KafkaEnv {..} = case envSaslMechanism of
  ...
  Just MechScramSha256 ->
    credMech
      "SCRAM-SHA-256"
      (SASL.SaslScram Scram.ScramSHA256)
      envSaslUsername
      envSaslPassword
  Just MechScramSha512 ->
    credMech
      "SCRAM-SHA-512"
      (SASL.SaslScram Scram.ScramSHA512)
      envSaslUsername
      envSaslPassword
