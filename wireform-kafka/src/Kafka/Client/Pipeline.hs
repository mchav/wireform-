...
import Kafka.Client.ReauthDriver qualified
import Kafka.Network.Connection (
  Connection,
  connectionClose,
  connectionPut,
 )
import Kafka.Network.Connection.Internal qualified as Conn (connDuplex)
import Kafka.Network.FrameParser qualified as FrameParser
...
import Wireform.Transport.Config (Profile (..), profileConfig)
    ...
    loop = do
      ...
      case mItem of
        ...
        Just (SendItem _cid bytes) -> do
          ...
          r <-
            try (connectionPut pipelineConnection framed)
              :: IO (Either SomeException ())
          case r of
            Left e -> failPipeline p ("send failed: " <> show e)
            Right () -> do
              atomically $ modifyTVar' pipelineStats $ \s ->
                s
