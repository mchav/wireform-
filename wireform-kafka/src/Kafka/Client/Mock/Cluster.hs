        ...
        | otherwise ->
            error $
              "createTopic: "
                <> T.unpack (id topic)
                <> " already exists with "
                <> show (IntMap.size (mtPartitions t))
                <> " partitions; cannot resize to "
                <> show n
      Nothing ->
      ...
      modifyTVar' (mcGroups c) $ \m ->
        let go !acc ((g, t, p), oam) =
              Map.insertWith
                Map.union
                g
                (Map.singleton (t, p) (oamOffset oam))
                acc
