...
{- |
Module      : Kafka.Client.Mock.Fault
...
import Kafka.Client.Mock.Cluster (GroupId, TxnId)
...
@errorMessage@ field (used by 'Kafka.Client.Mock.Fault').
-}
...
data FaultPolicy = FaultPolicy
  { fpProduceQ :: !(TVar (Map (Text, Int32) (Seq MockError)))
  , fpFetchQ :: !(TVar (Map (Text, Int32) (Seq MockError)))
  , fpCommitQ :: !(TVar (Map GroupId (Seq MockError)))
  , fpTxnQ :: !(TVar (Map (TxnId, TxnOp) (Seq MockError)))
  , fpStickyP :: !(TVar (Map (Text, Int32) MockError))
  , fpStickyF :: !(TVar (Map (Text, Int32) MockError))
  }
...
queueProduceErrors
  :: FaultPolicy -> Text -> Int32 -> [MockError] -> IO ()
queueProduceErrors fp t p errs =
  atomically $
...
addProduceFault :: FaultPolicy -> Text -> Int32 -> MockError -> IO ()
addProduceFault fp t p e = queueProduceErrors fp t p [e]


addFetchFault :: FaultPolicy -> Text -> Int32 -> MockError -> IO ()
...
setStickyProduce :: FaultPolicy -> Text -> Int32 -> MockError -> IO ()
setStickyProduce fp t p e =
  atomically $
    modifyTVar' (fpStickyP fp) (Map.insert (t, p) e)


setStickyFetch :: FaultPolicy -> Text -> Int32 -> MockError -> IO ()
setStickyFetch fp t p e =
  atomically $
    modifyTVar' (fpStickyF fp) (Map.insert (t, p) e)


clearStickyProduce :: FaultPolicy -> Text -> Int32 -> IO ()
clearStickyProduce fp t p =
  atomically $
    modifyTVar' (fpStickyP fp) (Map.delete (t, p))


clearStickyFetch :: FaultPolicy -> Text -> Int32 -> IO ()
clearStickyFetch fp t p =
  atomically $
...
-}
takeProduceFault
  :: FaultPolicy -> Text -> Int32 -> IO (Maybe MockError)
