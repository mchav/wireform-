...
finalClientMessage
  :: ScramSession
  -> ByteString
  -- ^ Raw server-first message (the bytes as received)
...
verifyServerFinal
  :: ByteString
  -- ^ Expected ServerSignature (raw bytes)
  -> ByteString
  -- ^ Raw server-final message ("v=...\" or "e=...\")
  -> Either String ()
verifyServerFinal expected raw =
  case BS.splitWith (== c ',') raw of
    (pair : _) -> case BS.splitAt 2 pair of
      ...
      _ ->
        Left
          ( "SCRAM: malformed server-final message: "
              <> BS8.unpack raw
          )
  ...
  where
    ...
    requiredAttr k kvs =
      case lookup k kvs of
        ...
        Nothing ->
          Left
            ( "SCRAM: server-first missing required attribute "
                <> BS8.unpack k
            )
    parseDecimal s
