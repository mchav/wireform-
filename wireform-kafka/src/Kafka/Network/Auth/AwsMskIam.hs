...
awsCredentialsFromEnv = do
  ...
  case (k, s) of
    (Just kk, Just ss) -> pure $ Right AwsCredentials
      { awsAccessKeyId     = T.pack kk
      , awsSecretAccessKey = T.pack ss
      , awsSessionToken    = T.pack <$> t
      }
    _ -> pure $ Left "AWS credentials: AWS_ACCESS_KEY_ID and \
...
data IamPayloadInput = IamPayloadInput
  ...
  , iiHost        :: !Text          -- ^ Broker hostname (no port)
  , iiRegion      :: !Text          -- ^ e.g. \"us-east-1\"
  , iiNow         :: !UTCTime       -- ^ "Now" in UTC; the signature is valid for 'iiExpires' seconds from this point
  , iiUserAgent   :: !Text          -- ^ Sent verbatim in the JSON; defaults are fine
  , iiExpires     :: !Int           -- ^ X-Amz-Expires; max 900 per AWS limits, default 900
  } deriving (Eq, Show)
...
canonicalQueryString
  :: Text          -- ^ Credential value: access-key/credential-scope
  -> Text          -- ^ X-Amz-Date in YYYYMMDDTHHMMSSZ
  -> Int           -- ^ X-Amz-Expires
  -> Maybe Text    -- ^ Optional X-Amz-Security-Token
  -> ByteString
canonicalQueryString credential amzDate expires sessionToken =
  let pairs0 :: [(ByteString, ByteString)]
      ...
      pairs = pairs0 <> case sessionToken of
        Nothing  -> []
...
stringToSign
  ...
  -> ByteString -- ^ Canonical request bytes
  -> ByteString
...
signingKey
  ...
  -> Text  -- ^ Datestamp YYYYMMDD
  -> Text  -- ^ Region
  -> Text  -- ^ Service (always \"kafka-cluster\" for AWS_MSK_IAM)
  -> ByteString
