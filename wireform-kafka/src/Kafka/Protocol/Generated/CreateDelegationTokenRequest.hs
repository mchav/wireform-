...
wirePokeCreatableRenewers version basePtr msg = do
  p0 <- pure basePtr
  p1 <- (if version >= 2 then WP.pokeCompactString p0 (P.toCompactString (creatableRenewersPrincipalType msg)) else WP.pokeKafkaString p0 (creatableRenewersPrincipalType msg))
  p2 <- (if version >= 2 then WP.pokeCompactString p1 (P.toCompactString (creatableRenewersPrincipalName msg)) else WP.pokeKafkaString p1 (creatableRenewersPrincipalName msg))
...
defaultCreatableRenewers :: CreatableRenewers
defaultCreatableRenewers = CreatableRenewers { creatableRenewersPrincipalType = P.KafkaString Null, creatableRenewersPrincipalName = P.KafkaString Null }
...
wirePokeCreateDelegationTokenRequest version basePtr msg
  ...
  | version == 3 = do
    p0 <- pure basePtr
    p1 <- (if version >= 3 then (if version >= 2 then WP.pokeCompactString p0 (P.toCompactString (createDelegationTokenRequestOwnerPrincipalType msg)) else WP.pokeKafkaString p0 (createDelegationTokenRequestOwnerPrincipalType msg)) else pure p0)
    p2 <- (if version >= 3 then (if version >= 2 then WP.pokeCompactString p1 (P.toCompactString (createDelegationTokenRequestOwnerPrincipalName msg)) else WP.pokeKafkaString p1 (createDelegationTokenRequestOwnerPrincipalName msg)) else pure p1)
...
wirePeekCreateDelegationTokenRequest version _fp _basePtr p0 endPtr
  ...
  | version == 3 = do
    (f0_ownerprincipaltype, p1) <- (if version >= 3 then (if version >= 2 then (\(cs, p') -> (P.fromCompactString cs, p')) <$> WP.peekCompactString p0 endPtr else WP.peekKafkaString p0 endPtr) else pure (P.KafkaString Null, p0))
    (f1_ownerprincipalname, p2) <- (if version >= 3 then (if version >= 2 then (\(cs, p') -> (P.fromCompactString cs, p')) <$> WP.peekCompactString p1 endPtr else WP.peekKafkaString p1 endPtr) else pure (P.KafkaString Null, p1))
