...
wirePokeMember version basePtr msg = do
  ...
  p2 <- WP.pokeCompactString p1 (P.toCompactString (memberRackId msg))
  p3 <- W.pokeInt32BE p2 (memberMemberEpoch msg)
  p4 <- WP.pokeCompactString p3 (P.toCompactString (memberClientId msg))
  p5 <- WP.pokeCompactString p4 (P.toCompactString (memberClientHost msg))
...
wirePeekMember version _fp _basePtr p0 endPtr = do
  ...
  (f1_rackid, p2) <- (\(cs, p') -> (P.fromCompactString cs, p')) <$> WP.peekCompactString p1 endPtr
  (f2_memberepoch, p3) <- W.peekInt32BE p2 endPtr
  (f3_clientid, p4) <- (\(cs, p') -> (P.fromCompactString cs, p')) <$> WP.peekCompactString p3 endPtr
  (f4_clienthost, p5) <- (\(cs, p') -> (P.fromCompactString cs, p')) <$> WP.peekCompactString p4 endPtr
