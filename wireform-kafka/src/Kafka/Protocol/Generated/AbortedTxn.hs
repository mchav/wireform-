...
-- | Encode AbortedTxn with the given API version.
encodeAbortedTxn :: MonadPut m => E.ApiVersion -> AbortedTxn -> m ()
encodeAbortedTxn version msg
  | version == 0 =
    do
      serialize (abortedTxnProducerId msg)
      serialize (abortedTxnFirstOffset msg)
      serialize (abortedTxnLastOffset msg)
      serialize (abortedTxnLastStableOffset msg)

  | otherwise = error $ "Unsupported version: " ++ show version

-- | Decode AbortedTxn with the given API version.
decodeAbortedTxn :: MonadGet m => E.ApiVersion -> m AbortedTxn
decodeAbortedTxn version
  | version == 0 =
    do
      fieldproducerid <- deserialize
      fieldfirstoffset <- deserialize
      fieldlastoffset <- deserialize
      fieldlaststableoffset <- deserialize
      pure AbortedTxn
        {
        abortedTxnProducerId = fieldproducerid
        ,
        abortedTxnFirstOffset = fieldfirstoffset
        ,
        abortedTxnLastOffset = fieldlastoffset
        ,
        abortedTxnLastStableOffset = fieldlaststableoffset
        }
