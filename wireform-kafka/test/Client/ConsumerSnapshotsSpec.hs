...
import Kafka.Client.Consumer qualified as CE
...
tests :: Spec
tests =
  describe "Consumer snapshots + helpers" $
    sequence_
      [ it
          "effectiveConsumerSnapshot reflects defaults"
          snapshot
      , it
          "withReadOnly clears auto-commit"
          readOnly
      , it
          "planRewind: Earliest -> low watermark"
          rewind_earliest
      , it
          "planRewind: Latest -> high watermark"
          rewind_latest
      , it
          "planRewind: Fail -> Nothing"
          rewind_fail
      , it
          "recordRebalanceTrigger surfaces canonical strings"
          trigger_strings
      , it
          "shutdownReasonText covers every constructor"
          shutdown_strings
      , it
          "assignorHintText canonical names"
          assignor_strings
      ]
