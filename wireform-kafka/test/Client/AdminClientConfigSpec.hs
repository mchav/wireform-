...
import Kafka.Client.AdminClient qualified as AE
import Test.Syd


tests :: Spec
tests =
  describe "AdminClient configuration helpers" $
    sequence_
      [ it
          "defaultAdminApiTimeoutMs matches the JVM client (60s)"
          timeout_default
      , it
          "defaultTopicCreateDefaults: 1 partition, 1 replica, no overrides"
          topic_defaults
      , it
          "defaultNullKeyCompactionPolicy = Reject"
          null_key_default
      , it
          "metric names follow the kafka.admin.* convention"
          metric_names
      ]
