...
tests :: Spec
tests =
  describe "ShareConsumer (KIP-932)" $
    sequence_
      [ it
          "defaults match the Java client (lock=30s, max-deliveries=5)"
          defaults
      , it
          "acknowledgements buffered + drained in order"
          ack_buffer
      , it
          "shouldRedeliver requires expiry AND non-poison"
          redeliver_logic
      , it
          "lockExpiresAt is locked-at + lock-timeout"
          lock_expires
      ]
