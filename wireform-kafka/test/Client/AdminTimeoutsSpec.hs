...
import Kafka.Client.AdminTimeouts qualified as A
import Test.Syd


tests :: Spec
tests =
  describe "AdminClient timeouts + routing (KIP-540 / 918 / 919)" $
    sequence_
      [ it
          "AdminUseDefault uses default.api.timeout.ms"
          use_default
      , it
          "AdminTimeoutMs overrides default"
          explicit_timeout
      , it
          "AdminNoDeadline returns Nothing"
          no_deadline
      , it
          "metadata reads -> RouteAnyBroker"
          metadata_route
      , it
          "topic / config / acl mutations -> RouteControllerBroker"
          mutation_route
      , it
          "broker / quorum lifecycle -> RouteKRaftQuorum"
          kraft_route
      ]
