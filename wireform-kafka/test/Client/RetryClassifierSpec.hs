...
tests :: Spec
tests =
  describe "Retry classifier (KIP-487 / 1054)" $
    sequence_
      [ it
          "code 0 -> ECNoError"
          no_error
      , it
          "transient codes are retriable"
          retriable
      , it
          "transactional / payload codes are abortable"
          abortable
      , it
          "auth + invalid-record codes are fatal"
          fatal
      , it
          "unknown codes default to retriable (forward-compat)"
          unknown_retriable
      , it
          "errorMessage uses canonical Kafka enum names"
          messages
      ]
...
retriable =
  mapM_
    (\c -> RC.classify c `shouldBe` RC.ECRetriable)
...
abortable =
  mapM_
    (\c -> RC.classify c `shouldBe` RC.ECAbortable)
...
fatal =
  mapM_
    (\c -> RC.classify c `shouldBe` RC.ECFatal)
