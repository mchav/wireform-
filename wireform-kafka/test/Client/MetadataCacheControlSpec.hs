...
tests :: Spec
tests = describe "MetadataCacheControl (KIP-294 / 526)" $ sequence_
  [ it "fresh age -> every topic needs refresh"
      fresh
  , it "after recordRefresh, topic stays fresh until threshold"
      after_refresh
  , it "topicsNeedingRefresh filters correctly"
      bulk
  , it "isStale matches the age check"
      stale_check
  ]
