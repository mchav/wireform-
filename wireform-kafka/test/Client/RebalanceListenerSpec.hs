...
import Kafka.Client.RebalanceListener qualified as RL
import Test.Syd


tests :: Spec
tests =
  describe "RebalanceListener (KIP-415 / 429)" $
    sequence_
      [ it
          "noop listener doesn't throw"
          noop
      , it
          "dispatchAssigned / Revoked / Lost route to right callbacks"
          routes
      , it
          "combineListeners runs both in order"
          combine
      , it
          "exceptions in listeners are swallowed"
          swallowed
      , it
          "computeAssignmentDelta: revoked + added are correct sets"
          deltaCorrect
      , it
          "computeAssignmentDelta: identical assignments yield empty deltas"
          deltaIdentity
      , it
          "computeAssignmentDelta: deterministic ascending order"
          deltaOrdered
      ]
