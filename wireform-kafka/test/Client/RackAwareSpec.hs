...
tests :: Spec
tests = describe "Rack-aware assignment (KIP-881)" $ sequence_
  [ it "rackAffinityScore: leader rack -> 100"
      score_leader
  , it "rackAffinityScore: replica rack -> 50"
      score_replica
  , it "rackAffinityScore: no match -> 0"
      score_none
  , it "preferLocalRack filters by leader-rack match"
      filter_local
  , it "rackAwareAssignment respects target load + prefers same rack"
      rack_assign
  ]
