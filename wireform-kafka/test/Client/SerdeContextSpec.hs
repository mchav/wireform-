...
import Kafka.Client.SerdeContext qualified as SC
import Test.Syd


tests :: Spec
tests =
  describe "SerdeContext (KIP-492)" $
    sequence_
      [ it
          "liftSerdeCtx ignores the context"
          lift_ignores
      , it
          "context-aware encoder sees topic"
          context_visible
      , it
          "withTopic / withHeaders helpers"
          builders
      ]
