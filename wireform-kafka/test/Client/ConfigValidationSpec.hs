    ...
    sequence_
      ...
      , describe "Consumer" $
          sequence_
            ...
            , it
                "heartbeat.interval.ms >= session.timeout.ms is rejected"
                prop_consumerHeartbeatTooHighRejected
            , it
                "max.poll.interval.ms < session.timeout.ms is rejected"
                prop_consumerPollIntervalTooLowRejected
            , it
                "fetch.min.bytes > fetch.max.bytes is rejected"
                prop_consumerFetchMinExceedsMaxRejected
            , it
                "auto-commit interval must be > 0 when auto-commit is on"
                prop_consumerAutoCommitIntervalRequiredWhenEnabled
            , it
                "auto-commit interval is ignored when auto-commit is off"
