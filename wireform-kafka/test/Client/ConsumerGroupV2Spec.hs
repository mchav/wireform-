...
tests :: Spec
tests = describe "ConsumerGroupV2 (KIP-848)" $ sequence_
  [ describe "transitionMemberState" $ sequence_
      [ it "Unsubscribed -> Joining ok"
          (G.transitionMemberState G.MSUnsubscribed G.MSJoining `shouldBe` Just G.MSJoining)
      , it "Joining -> Stable ok"
          (G.transitionMemberState G.MSJoining G.MSStable `shouldBe` Just G.MSStable)
      , it "Stable -> Reconciling ok"
          (G.transitionMemberState G.MSStable G.MSReconciling `shouldBe` Just G.MSReconciling)
      , it "Stable -> Unsubscribed rejected"
          (G.transitionMemberState G.MSStable G.MSUnsubscribed `shouldBe` Nothing)
      , it "Anything -> Fenced ok"
          (G.transitionMemberState G.MSStable G.MSFenced `shouldBe` Just G.MSFenced)
      , it "Fenced -> Unsubscribed ok"
          (G.transitionMemberState G.MSFenced G.MSUnsubscribed `shouldBe` Just G.MSUnsubscribed)
      ]
  , describe "planHeartbeat" $ sequence_
      [ it "fenced -> MSFenced + every previous partition lost"
          fenced_path
      , it "first assignment -> MSStable, all delta in adAssigned"
          first_assignment
      , it "no change -> stays in same state, empty delta"
          steady_state
      , it "new partitions -> MSReconciling + adAssigned set"
          gain_partitions
      , it "lost partitions -> MSReconciling + adRevoked set"
          lose_partitions
      , it "next-heartbeat-ms = now + cadence"
          next_hb_arithmetic
      ]
  , describe "subscription helpers" $ sequence_
      [ it "defaultSubscription uses 45_000ms session timeout"
