...
import Kafka.Telemetry.Push qualified as P
import Test.Syd


tests :: Spec
tests =
  describe "Telemetry push (KIP-714)" $
    sequence_
      [ it
          "no subscription -> RefreshSubscription"
          no_sub
      , it
          "stale subscription -> Refresh"
          stale_sub
      , it
          "subscription valid + push interval elapsed -> Push"
          time_to_push
      , it
          "valid subscription, before interval -> Sleep"
          not_yet
      , it
          "terminating -> Done"
          terminating
