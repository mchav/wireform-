...
import Kafka.Client.Transaction qualified as PE
import Test.Syd
...
tests =
  describe "Transaction helpers" $
    sequence_
      [ it
          "transactionalIdOptional override wins"
          txn_override
      , it
          "transactionalIdOptional defaults to prefix-suffix"
          txn_default
      , it
          "classifyTxnError: retriable code -> Retry"
          classify_retry
      , it
          "classifyTxnError: abortable code -> Abort"
          classify_abort
      , it
          "classifyTxnError: fatal code -> Fatal"
          classify_fatal
      , it
          "effectiveTxnDeadlineMs: explicit deadline wins"
          explicit_deadline
      , it
          "effectiveTxnDeadlineMs: producer default fallback"
          default_deadline
      ]
