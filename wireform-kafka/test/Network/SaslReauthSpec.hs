...
tests :: Spec
tests = describe "SASL re-auth (KIP-368)" $ sequence_
  [ describe "effectiveReauthDeadlineMs" $ sequence_
      [ it "no deadlines configured -> Nothing"
          d_no_deadlines
      , it "broker lifetime only -> now + lifetime"
          d_broker_only
      , it "client max only -> now + max"
          d_client_only
      , it "both set -> now + min(broker, client)"
          d_both_set
      , it "broker > client -> client wins"
          d_broker_larger
      , it "broker < client -> broker wins"
          d_broker_smaller
      ]
  , describe "reauthRequiredAtMs" $ sequence_
      [ it "Nothing deadline -> never"
          r_never
      , it "deadline far in the future -> not required"
          r_far_future
      , it "deadline within the safety margin -> required"
          r_within_margin
      , it "deadline already passed -> required"
          r_passed
      , it "safety margin >= 1 second"
          r_min_margin
      ]
  ]
