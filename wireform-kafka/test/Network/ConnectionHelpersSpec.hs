...
import Kafka.Network.Connection qualified as CX
import Test.Syd
...
tests =
  describe "Connection helpers" $
    sequence_
      [ it
          "shouldDelayConnect = Nothing when no throttle recorded"
          no_throttle
      , it
          "shouldDelayConnect returns remaining wait after recordThrottle"
          throttle_set
      , it
          "isIdle returns True for fresh + unrecorded keys"
          idle_unrecorded
      , it
          "recordActivity prevents idle until threshold passes"
          activity_then_idle
      , it
          "prioritise sorts critical first, low last"
          prioritise_order
      , it
          "defaultSaslTimeouts: 30s connect, 0 idle"
          sasl_defaults
      ]
