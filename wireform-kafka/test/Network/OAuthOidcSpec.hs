...
import Kafka.Network.Auth.OAuthOidc qualified as O
...
tests :: Spec
tests =
  describe "OAuth/OIDC + PKCE (KIP-768 / KIP-1169)" $
    sequence_
      [ it
          "PKCE plain method passes the verifier through"
          pkce_plain
      , it
          "PKCE S256 produces a non-empty url-safe digest"
          pkce_s256
      , it
          "tokenRefreshDeadlineMs uses 75% of remaining lifetime"
          refresh_deadline
      , it
          "shouldRefreshToken trips before expiry"
          should_refresh
      , it
          "TokenCache stores + retrieves"
          cache_round_trip
      , it
          "oidcTokenProvider reuses a fresh cached token"
          provider_uses_fresh_cache
      , it
          "oidcTokenProvider fetches and stores a stale token"
          provider_refreshes_stale_cache
      , it
          "oidcTokenProvider requires a PKCE verifier when enabled"
