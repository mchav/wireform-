...
import Kafka.Network.Transport qualified as T
import Test.Syd


tests :: Spec
tests =
  describe "Kafka.Network.Transport" $
    sequence_
      [ it
          "pipe transport: write on one side -> read on the other"
          pipe_round_trip
      , it
          "pipe transport: partial reads return only requested bytes"
          pipe_partial_reads
      , it
          "pipe transport: closing surfaces EOF on next read"
          pipe_close_eof
      , it
          "pipe transport: write after close returns Left"
          pipe_write_after_close
      , it
          "transportName labels are distinct for the two sides"
          pipe_names_distinct
      ]
