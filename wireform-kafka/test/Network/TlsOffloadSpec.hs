...
-- 'Sink'.
withTcpEcho
  :: Sink                     -- ^ where to accumulate received bytes
  -> (Int -> IO a)            -- ^ action gets the bound port
  -> IO a
withTcpEcho sink act =
  bracket
    ...
    Socket.close $ \srv -> do
      ...
      Socket.bind srv (Socket.SockAddrInet 0
...
static_tcp_offload_routes_to_sidecar = do
  ...
  withTcpEcho sink $ \sidecarPort -> do
    let offload = Offload.staticTlsOffload
                    (Offload.TlsOffloadTcp "127.0.0.1" (fromIntegral sidecarPort))
        cfg = Conn.defaultConnectionConfig
