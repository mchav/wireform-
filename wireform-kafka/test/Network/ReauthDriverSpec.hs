...
import Data.IORef
import Kafka.Client.ReauthDriver qualified as RD
...
tests :: Spec
tests =
  describe "Reauth driver (KIP-368)" $
    sequence_
      [ it
          "newly created state has no deadline + no in-flight handshake"
          fresh_state
      , it
          "forceReauthNow triggers a handshake on the next driver tick"
          force_now
      , it
          "successful runner records the broker lifetime and a new deadline"
          success_path
      , it
          "failing runner records the error and clears in-flight"
          failure_path
      , it
          "awaitReauthQuiet returns immediately when no handshake is in flight"
          quiet_returns_now
      ]
