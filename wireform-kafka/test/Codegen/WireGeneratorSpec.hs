...
tests :: Spec
tests =
  describe "WireGenerator (codegen snapshot)" $
    sequence_
      [ describe "RequestHeader" $
          sequence_
            [ it
                "isWireSupported = True"
                (assertSupported requestHeader)
            , it
                "renders wireMaxSize / wirePoke / wirePeek"
                (assertRendersFunctions requestHeader)
            , it
                "uses pokeKafkaString for ClientId (flexibleVersions: none opt-out)"
                ( assertRenderedContains
                    requestHeader
            ...
            , it
                "v2 emits the empty tagged-fields trailer"
                ( assertRenderedContains
                    requestHeader
            ...
            , it
                "WireCodec override points at the natives"
                ( assertOverrideContains
                    requestHeader
      ...
      , describe "ResponseHeader" $
          sequence_
            ...
            , it
                "v0 has no tagged-fields trailer; v1 does"
                ( assertRenderedContains
                    responseHeader
                    [ "version == 0"
                    , "version == 1"
      ...
      , describe "ApiVersionsRequest" $
          sequence_
            ...
            , it
                "v3-4 uses pokeCompactString for both string fields"
                ( assertRenderedContains
                    apiVersionsRequest
            ...
            , it
                "v0-2 decode falls through to defaulted KafkaString Null"
                ( assertRenderedContains
                    apiVersionsRequest
                    ["P.KafkaString Null"]
                )
...
assertSupported sch =
  ( if (isWireSupported sch)
      then pure ()
      else
        expectationFailure
          ( "isWireSupported should be True for "
...
assertSubstring sch hay needle =
...
mkField
  ...
  -- ^ versions
  -> Maybe Text
  -- ^ flexibleVersions override
  -> Maybe Text
  -- ^ nullableVersions
  -> FieldSpec
mkField n t vs flex nul =
  FieldSpec
    { fieldName = n
    , fieldType = t
    , fieldVersions = vs
