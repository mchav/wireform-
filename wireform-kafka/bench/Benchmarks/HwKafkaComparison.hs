...
import Kafka.Client.Consumer qualified as WC
import Kafka.Client.Producer qualified as WP
import Kafka.Consumer qualified as HWC
import Kafka.Producer qualified as HW
...
benchmarks =
  bgroup
    ...
    [ case envBroker of
        ...
        Just _broker ->
          bgroup
            ("1 partition, " ++ show recordsPerRun ++ " records / iteration")
...
hwSetup = do
  let !topic = "wireform-bench-cmp" -- pre-create on the broker
      ...
      !props =
        HW.brokersList [HW.BrokerAddress (T.pack broker)]
...
hwConsumerSetup = do
  ...
  let !broker = fromMaybe (error "WIREFORM_KAFKA_BROKER unset") envBroker
      !cfg =
        HWC.brokersList [HWC.BrokerAddress (T.pack broker)]
          <> HWC.groupId (HWC.ConsumerGroupId (T.pack groupId))
          <> HWC.noAutoCommit
          <> HWC.logLevel HWC.KafkaLogErr
      !sub =
        HWC.topics [HWC.TopicName consumerTopic]
