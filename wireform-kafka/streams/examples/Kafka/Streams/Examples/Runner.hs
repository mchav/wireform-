...
withDemoDriver
  :: RunMode
  -> Text                        -- ^ application id
  -> IO Topology                 -- ^ topology builder
  -> [DemoTopic]                 -- ^ input topics (consumed by topology)
  -> [DemoTopic]                 -- ^ output topics (sinks the demo will drain)
  -> (DemoDriver -> IO ())
  -> IO ()
...
crFromConsumer cr = CollectedRecord
...
brokerOnlyWarning name = \case
  InMemory -> pure ()
  Broker _ -> hPutStrLn stderr $
    "wireform-kafka-streams-examples: '" <> name
