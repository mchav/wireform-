...
import Kafka.Streams.Examples.Ops.Helpers (section, bullet)
...
runDemo = do
  ...
  active <- loggedKeyValueStore activeUnder changelog
              (storeName "session-store") textSerde textSerde

  -- Standby side: an independent store + a 'StandbyTask' that
  -- catches it up off the changelog.
  standbyStore <- inMemoryKeyValueStore @Text @Text
                    (storeName "session-store-standby")
  sb <- newStandbyTask standbyStore changelog
          (storeName "session-store") textSerde textSerde
  ...
  bullet ("Active is now writing; acceptableRecoveryLag = "
            <> show acceptableLag)

  -- Three rounds: the active emits writes, then we advance the
  -- standby, then we check readiness.
  forM_ [(1 :: Int, mkBatch 1 5),
         (2,       mkBatch 6 5),
         (3,       mkBatch 11 2)] $ \(round_, batch) -> do
    ...
    let progress = PR.WarmupProgress
          { PR.task = TaskId 0 0
          , PR.lag  = lag
          }
