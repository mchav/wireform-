...
runDemo = do
  ...
  bullet
    ( "Changelog rebuild: applied "
        <> show (backfillApplied res1)
        <> " entries over offsets ["
        <> show (backfillFromOffset res1)
        <> ","
  ...
  case res2 of
    ...
    Right br -> do
      bullet
        ( "Snapshot + tail: restored snapshot, replayed tail from offset "
  ...
  bullet
    ( "CDC drain: applied "
        <> show (cdcApplied res3)
        <> " compacted events in "
        <> show (cdcSteps res3)
        <> " step(s), final phase "
        <> show (cdcFinalPhase res3)
    )
