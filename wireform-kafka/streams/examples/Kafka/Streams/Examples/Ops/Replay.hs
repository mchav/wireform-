...
import Kafka.Streams.Types qualified as KT
...
runDemo = do
  ...
  let outHeaderKeys =
        concatMap
          (\(_, crs) -> concatMap decodeHeaderKeys crs)
  ...
  let offsetLog = zipWith replayWithOffset [0 ..] capturedLog
      offPlan =
        defaultReplayPlan
          { replayFromOffset = Just 1
          , replayToOffset = Just 3
          }
