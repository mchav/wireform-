...
import Kafka.Client.RebalanceListener qualified as RBL
...
import Kafka.Streams.Processor.Assignment qualified as PA
...
import Kafka.Streams.Runtime.ProbingRebalance qualified as ProbingRebalance
...
feedWithHandler
  :: KafkaStreams
  -> Text
  -- ^ node name where the work runs
  -> KC.ConsumerRecord
  -> IO ()
  -- ^ the actual feed action
  -> IO ()
...
cleanUp ks = do
  ...
  case st of
    ...
    other ->
      error $
        "cleanUp: runtime not stopped (current state: "
          <> show other
          <> ")"
  where
    doCleanUp = do
...
streamsApplicationState ks = do
        ...
        Map.fromList
          [ ( TaskId 0 tp.partition
            , PA.TaskInfo
                ...
                , PA.tiTopicPartitions =
                    Set.singleton
                      ( PA.TaskTopicPartition
  ...
  pure
    PA.ApplicationState
      ...
      , PA.asAssignmentConfigs =
          PA.AssignmentConfigs
