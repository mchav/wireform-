...
reduceKGroupedTable
  :: forall k v
   . Ord k
  => (v -> v -> v)             -- ^ adder    (oldAgg new -> newAgg)
  -> (v -> v -> v)             -- ^ subtractor (oldAgg old -> partial)
  -> Materialized k v
  -> KGroupedTable k v
...
reduceKGroupedTable add sub m kgt = do
  ...
  let outBuilder  = inMemoryKeyValueStoreBuilder storeNm
                      :: StoreBuilderKV k v
      prevBuilder = inMemoryKeyValueStoreBuilder prevNm
                      :: StoreBuilderKV k v
  ...
  pure Processor
    ...
    , procInit = \ctx -> do
        ...
        getStateStore ctx outNm >>= \case
          ...
          _ -> error $ "KGroupedTable.reduce: out store missing: "
                       <> show outNm
        getStateStore ctx prevNm >>= \case
          Just (AnyKeyValueStore kvs) ->
            writeIORef prevRef (Just (Unsafe.unsafeCoerce kvs))
          _ -> error $ "KGroupedTable.reduce: prev store missing: "
                       <> show prevNm
    , procClose = pure ()
  ...
  pure Processor
    ...
    , procInit = \ctx -> do
        ...
        getStateStore ctx outNm >>= \case
          Just (AnyKeyValueStore kvs) ->
            writeIORef outRef (Just (Unsafe.unsafeCoerce kvs))
          _ -> error $ "KGroupedTable.aggregate: out store missing: "
                       <> show outNm
        getStateStore ctx prevNm >>= \case
          Just (AnyKeyValueStore kvs) ->
            writeIORef prevRef (Just (Unsafe.unsafeCoerce kvs))
          _ -> error $ "KGroupedTable.aggregate: prev store missing: "
                       <> show prevNm
    , procClose = pure ()
    , procProcess = \r -> case recordKey r of
        Nothing -> pure ()
