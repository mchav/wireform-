...
import Kafka.Streams.AsyncIO qualified as AIO
import Kafka.Streams.AsyncIO.Config qualified as AIO
...
import Kafka.Streams.ForeignKeyJoin qualified as FK
...
import Kafka.Streams.Joined qualified as Joined
import Kafka.Streams.KGroupedStream (
  KGroupedStream,
  SessionWindowedKStream,
  TimeWindowedKStream,
 )
import Kafka.Streams.KGroupedStream qualified as KGS
import Kafka.Streams.KGroupedTable (KGroupedTable)
import Kafka.Streams.KGroupedTable qualified as KGT
...
import Kafka.Streams.Topology.Free.Arrow qualified as FA
import Kafka.Streams.Types (Record (..), TopicName, topicName)
import Kafka.Streams.Window qualified as Win
import System.IO.Unsafe qualified as IOUnsafe
...
addReadOnlyStateStore
  ...
  -- ^ source topic
  -> Serde k
  -- ^ key serde of the source topic
  -> Serde v
  -- ^ value serde of the source topic
  -> Time.TimestampExtractor k v
  -> Topo.AnyProcessor
  -- ^ updater body
  -> Topology x x
...
labelPrim p0 = case p0 of
  Split bs md ->
    "Split("
      <> T.intercalate "|" (Prelude.map sbName bs)
...
collapseFirst cfg (Arr f)
  | optFusePureFunctions cfg =
      Arr (\(a, c) -> (f a, c))
...
collapseParallel cfg (Arr f) (Arr g)
  | optFusePureFunctions cfg =
      Arr (\(a, c) -> (f a, g c))
...
collapseFanout
  ...
  -> Topology a c
  -> Topology a (b, c)
collapseFanout cfg (Arr f) (Arr g)
  | optFusePureFunctions cfg =
      Arr (\a -> (f a, g a))
...
collapseLeft
  :: OptimizeConfig
  -> Topology a b
  -> Topology (Either a c) (Either b c)
...
collapseRight
  :: OptimizeConfig
  -> Topology a b
  -> Topology (Either c a) (Either c b)
...
collapsePlus
  ...
  -> Topology a b
  -> Topology c d
  -> Topology (Either a c) (Either b d)
...
fuseStep cfg g f = case (g, f) of
  ...
  -- MapValues family
  (Lift (MapValues g'), Lift (MapValues f'))
    | optFuseMaps cfg ->
        Just (liftPrim (MapValues (g' . f')))
  (Lift (MapValuesM g'), Lift (MapValues f'))
    | optFuseMaps cfg ->
        Just (liftPrim (MapValuesM (g' . f')))
  (Lift (MapValues g'), Lift (MapValuesM f'))
    | optFuseMaps cfg ->
        Just (liftPrim (MapValuesM (\v -> g' <$> f' v)))
  (Lift (MapValuesM g'), Lift (MapValuesM f'))
    | optFuseMaps cfg ->
        Just (liftPrim (MapValuesM (\v -> f' v >>= g')))
  -- MapKeyValue family
  (Lift (MapKeyValue g'), Lift (MapKeyValue f'))
    | optFuseMaps cfg ->
        Just
          ( liftPrim
              ( MapKeyValue
  ...
  (Lift (MapKeyValueM g'), Lift (MapKeyValue f'))
    | optFuseMaps cfg ->
        Just
          ( liftPrim
              ( MapKeyValueM
  ...
  (Lift (MapKeyValue g'), Lift (MapKeyValueM f'))
    | optFuseMaps cfg ->
        Just
          ( liftPrim
              ( MapKeyValueM
  ...
  (Lift (MapKeyValueM g'), Lift (MapKeyValueM f'))
    | optFuseMaps cfg ->
        Just
          ( liftPrim
              ( MapKeyValueM
  ...
  -- its 'AsyncIOConfig'.
  (Lift (AsyncMapValues aioCfg g'), Lift (MapValues f'))
    | optFuseSyncIntoAsync cfg ->
        Just (liftPrim (AsyncMapValues aioCfg (\v -> g' (f' v))))
  (Lift (AsyncMapKeyValue aioCfg g'), Lift (MapKeyValue f'))
    | optFuseSyncIntoAsync cfg ->
        Just
          ( liftPrim
              ( AsyncMapKeyValue
  ...
  (Lift (AsyncConcatMapValues aioCfg g'), Lift (MapValues f'))
    | optFuseSyncIntoAsync cfg ->
  ...
  (Lift (MapRecord g'), Lift (MapRecord f'))
    | optFuseMaps cfg ->
        Just (liftPrim (MapRecord (g' . f')))
  (Lift (MapRecordM g'), Lift (MapRecord f'))
    | optFuseMaps cfg ->
        Just (liftPrim (MapRecordM (g' . f')))
  (Lift (MapRecord g'), Lift (MapRecordM f'))
    | optFuseMaps cfg ->
        Just (liftPrim (MapRecordM (\r -> g' <$> f' r)))
  (Lift (MapRecordM g'), Lift (MapRecordM f'))
    | optFuseMaps cfg ->
        Just (liftPrim (MapRecordM (\r -> f' r >>= g')))
  -- FlatMap fusion
  (Lift (ConcatMapValues g'), Lift (MapValues f'))
    | optFuseConcatMaps cfg ->
        Just (liftPrim (ConcatMapValues (g' . f')))
  (Lift (MapValues g'), Lift (ConcatMapValues f'))
    | optFuseConcatMaps cfg ->
        Just (liftPrim (ConcatMapValues (fmap g' . f')))
  (Lift (ConcatMapValues g'), Lift (ConcatMapValues f'))
    | optFuseConcatMaps cfg ->
  ...
  (Lift (Filter p2), Lift (Filter p1))
    | optFuseFilters cfg ->
        Just (liftPrim (Filter (\r -> p1 r && p2 r)))
  (Lift (FilterNot p2), Lift (FilterNot p1))
    | optFuseFilters cfg ->
        Just (liftPrim (FilterNot (\r -> p1 r || p2 r)))
  (Lift (Filter p), Lift (FilterNot q))
    | optFuseFilters cfg ->
        Just (liftPrim (Filter (\r -> not (q r) && p r)))
  (Lift (FilterNot q), Lift (Filter p))
    | optFuseFilters cfg ->
  ...
  (Lift (SelectKey g'), Lift (SelectKey f'))
    | optFuseSelectKeys cfg ->
        Just
          ( liftPrim
              ( SelectKey
                  ( \r ->
  ...
  (Lift (GroupByKey g'), Lift (SelectKey f'))
  ...
  ( Lift (GroupByKey g')
    , Compose
        (Lift (RepartitionWith _))
        (Compose (Lift (SelectKey f')) rest)
    )
      | optFuseSelectKeyIntoGroupBy cfg ->
          Just (Compose (liftPrim (GroupBy f' g')) rest)
  -- Repartition idempotence
  (Lift (Repartition pfx2), Lift (Repartition _))
    | optCollapseRepartition cfg ->
        Just (liftPrim (Repartition pfx2))
  (Lift (RepartitionWith cfg2), Lift (Repartition _))
    | optCollapseRepartition cfg ->
        Just (liftPrim (RepartitionWith cfg2))
  (Lift (Repartition pfx), Lift (RepartitionWith _))
    | optCollapseRepartition cfg ->
        Just (liftPrim (Repartition pfx))
  (Lift (RepartitionWith cfg2), Lift (RepartitionWith _))
    | optCollapseRepartition cfg ->
        Just (liftPrim (RepartitionWith cfg2))
