...
registerSchemaRequest
  :: Text                         -- ^ base url, e.g. @http://schemas.example.com@
  -> SR.SchemaSubject
  -> SR.SchemaPayload
  -> HttpRequest
registerSchemaRequest baseUrl subj payload = HttpRequest
  { reqMethod  = HttpPost
  , reqUrl     = baseUrl
                  <> "/subjects/" <> SR.unSchemaSubject subj
                  <> "/versions"
...
lookupSchemaRequest baseUrl sid = HttpRequest
  { reqMethod  = HttpGet
  , reqUrl     = baseUrl
                  <> "/schemas/ids/"
...
lookupBySubjectRequest baseUrl subj = HttpRequest
  { reqMethod  = HttpGet
  , reqUrl     = baseUrl
                  <> "/subjects/" <> SR.unSchemaSubject subj
                  <> "/versions/latest"
...
httpBackedRegistry baseUrl requester = SR.SchemaRegistryClient
