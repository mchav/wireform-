...
@org.apache.kafka.common.serialization.Serdes@:
  ...
  * 'byteStringSerde' — opaque bytes
  * 'textSerde' — UTF-8 'Text'
  * 'utf8Serde' — UTF-8 'String' (allocates; prefer 'textSerde')
  * 'int32Serde' / 'int64Serde' / 'doubleSerde' — big-endian fixed-width
...
Constructors:
  ...
  * 'prefixedSerde' — length-prefixed with a 32-bit big-endian header
-}
module Kafka.Streams.Serde (
  -- * Type
  Serde (..),
...
{- | Bidirectional codec.  'deserialize' returns 'Either' so source
processors can route failures through 'DeserializationHandler'.
-}
data Serde a = Serde
...
{- | Convenience: serialise a 'Kafka.Streams.Types.Record' through a
