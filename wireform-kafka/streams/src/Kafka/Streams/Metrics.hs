...
observeDuration r nm d = atomically $
  modifyTVar' (mrDurations r) (Map.alter step nm)
  where
    step Nothing = Just DurationStats
      { dsCount = 1
      , dsSum   = d
      , dsMin   = d
      , dsMax   = d
      }
    step (Just cur) = Just DurationStats
