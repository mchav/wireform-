      ...
      Left e ->
        putStrLn $
          "[streams] processor close failed ("
            <> T.unpack (unProcessorName (prName pr))
            <> "): "
            <> show e
      _ -> pure ()
      ...
      Left e ->
        putStrLn $
          "[streams] store close failed ("
            <> T.unpack (unStoreName (storeEntryName se))
            <> "): "
            <> show e
      _ -> pure ()
