...
mkStore
  ...
  -> SessionStore k v
mkStore nm retention ref = SessionStore
  { ssBase = StateStore
      { storeStoreName  = nm
  ...
  , ssFindSessions = \k earliestEnd latestStart -> do
      m <- readIORef ref
      let !sub  = Map.takeWhileAntitone
                    (\(e, _, _) -> e <= maxBoundT)
                $ Map.dropWhileAntitone
                    (\(e, _, _) -> e <  earliestEnd) m
          !out = [ (SessionKey k' s e, v)
...
inMemorySessionStoreBuilder
  ...
  -> Int64
  -> StoreBuilderS k v
inMemorySessionStoreBuilder nm retention = StoreBuilderS
  { sbSName    = nm
