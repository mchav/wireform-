...
data CommitOutcome
  = CommitSucceeded
  | -- | Aborted due to an error; runtime should rebalance
    CommitAborted !Text
  | -- | Fatal error; runtime should fail-fast
    CommitFatal !Text
...
withTransactionalStores
  ...
  -> EOSCoordinator
withTransactionalStores base commits aborts =
  base
