...
shouldSnapshot
  :: SnapshotPlan
  -> Timestamp                       -- ^ wall-clock now
  -> Timestamp                       -- ^ last snapshot at
  -> Int64                           -- ^ records consumed since last snapshot
...
shouldSnapshot plan now lastAt recordsSince
  ...
  , recordsSince >= lim = Just TriggerRecordCount
  | otherwise = Nothing
...
publishIfDue
  ...
  -> ObjectStoreClient
  -> StoreName
  -> Timestamp                                -- ^ now (wall-clock)
  -> Int64                                    -- ^ current changelog offset
  -> (k -> ByteString)
  -> (v -> ByteString)
  -> KeyValueStore k v
...
pruneOldSnapshots
  ...
  -> StoreName
  -> Int                        -- ^ keep this many
  -> IO (Either Text Int)       -- ^ deleted count
