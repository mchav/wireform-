...
data LagSeverity
  = LagCaughtUp           -- ^ Every task is at its end offset.
  | LagWithinThreshold    -- ^ Some lag, but the max is within budget.
  | LagExceeded           -- ^ The max lag exceeds the threshold.
...
renderLagTable rep =
  ...
  where
    ...
    summary =
      "tasks=" <> T.pack (show (lagReportTaskCount rep))
        <> " caughtUp=" <> T.pack (show (lagReportCaughtUp rep))
        <> " maxBehind=" <> T.pack (show (lagReportMaxBehind rep))
        <> " totalBehind=" <> T.pack (show (lagReportTotalBehind rep))
...
recordLagReport reg rep = do
  ...
  where
    recordTask tl =
      setGauge reg (lagGaugeName (taskLagId tl))
