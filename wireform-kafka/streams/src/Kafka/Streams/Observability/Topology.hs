...
liveTopologyDescription topo metrics cfg = do
  ...
  let base = topologyDescriptionWith cfg topo
      metricsObj =
        A.Object
          (KM.fromList
            (map (\(k, v) -> (AK.fromText k, renderMetricValue v))
                 (Map.toList dump)))
  pure $ case base of
...
renderSources topo =
  A.Array $ V.fromList
    [ A.Object (KM.fromList
        ...
        , ("topics",       A.Array (V.fromList
                              (map (A.String . unTopicName)
                                   (Topo.sourceTopics s))))
...
renderProcessors topo =
  A.Array $ V.fromList
    [ A.Object (KM.fromList
        ...
        , ("inputs",   A.Array (V.fromList
                          (map (A.String . unNodeName)
                               (Topo.processorSpecParents p))))
        , ("outputs",  childList topo (Topo.processorSpecName p))
        , ("stores",   A.Array (V.fromList
                          (map (A.String . Store.unStoreName)
