...
data OrphanReason
  = OrphanChangelog     -- ^ Topic name ends in @-changelog@.
  | OrphanRepartition   -- ^ Topic name ends in @-repartition@.
...
detectOrphans
  :: Topo.Topology
  -> Text                -- ^ application id
  -> [TopicName]         -- ^ topics the broker has
  -> [OrphanInternalTopic]
      ...
      classify t
        ...
        | otherwise =
            case suffixedReason t of
              Just r | not (Set.member t expected) ->
