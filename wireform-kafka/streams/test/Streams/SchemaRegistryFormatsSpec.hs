...
import Kafka.Streams.Serde qualified as Serde
import Kafka.Streams.Serde.Avro qualified as Avro
import Kafka.Streams.Serde.JsonSchema qualified as JS
import Kafka.Streams.Serde.Protobuf qualified as PB
...
tests :: Spec
tests =
  describe "Schema-Registry payload serdes" $
    sequence_
      [ it
          "Avro round-trip through the in-memory registry"
          avro_round_trip
      , it
          "JSON-Schema round-trip"
          json_round_trip
      , it
          "Protobuf round-trip with message-index 0"
          proto_round_trip_idx0
      , it
          "Protobuf round-trip with message-index 5"
          proto_round_trip_idxN
      , it
          "encodeMessageIndex(0) is the single byte 0x00"
          idx_zero
      , it
          "encodeMessageIndex / decodeMessageIndex round-trip"
          idx_round_trip
      ]
