...
tests :: Spec
tests =
  describe "KIP-869 task revocation grace" $
    sequence_
      [ it
          "grace = 0 -> RevokeImmediate"
          grace_zero_immediate
      , it
          "grace > 0 -> KeepAsStandby with deadline"
          grace_keeps_standby
      , it
          "negative grace -> RevokeImmediate"
          grace_negative
      , it
          "planRevocation maps over a list of tasks"
          plan_maps
      ]
