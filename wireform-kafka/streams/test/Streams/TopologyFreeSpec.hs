...
import Kafka.Streams.Topology.Free.Graphviz qualified as DOT
...
tests :: Spec
tests =
  describe "Topology.Free (GADT topology builder)" $
    sequence_
...
test_tap_passes_wire_through =
  it "Tap forks a side pipeline and keeps the main lineage flowing" $ do
    ...
    map (unbytes . crValue) main_
      `shouldBe` ["AUDIT:LOGIN", "REGULAR", "AUDIT:LOGOUT"]
    closeDriver driver
  ...
  it "Cat.id . op . Cat.id collapses; first/second/parallel of Id collapse" $ do
    ...
    ( if (before - after >= 2)
        then pure ()
          ...
          expectationFailure
            ( "expected at least 2 nodes saved; before="
                <> show before
                <> " after="
                <> show after
            )
  ...
  it "Fanout/Parallel/First of pure functions collapse to a single Arr" $ do
    ...
    ( if (before > 1 && after == 1)
        then pure ()
          ...
          expectationFailure
            ( "expected pure-function chain to collapse; before="
                <> show before
                <> " after="
                <> show after
            )
  ...
  it "default compile reduces topology node count vs compileNoOptimize" $ do
    ...
    ( if (nOpt < nNoOpt)
        then pure ()
          ...
          expectationFailure
            ( "optimised compile should produce fewer nodes; opt="
                <> show nOpt
                <> " noopt="
                <> show nNoOpt
            )
  ...
  it "Arr f . Fork collapses to a single Arr" $ do
    ...
    ( if (after == 1 && before > 1)
        then pure ()
          ...
          expectationFailure
            ( "expected Arr.Fork to collapse; before="
                <> show before
                <> " after="
                <> show after
            )
...
test_missing_serde_throws_typed_exception =
  it "forcing an aggregation KTable's missing serde raises TopologyFreeError" $ do
    ...
    case result of
      ...
      Left other ->
        error
          ( "expected MissingMaterializedSerde ValueSide, got: "
              <> show other
          )
      Right _ ->
  ...
  it "dimap pre/post-composes pure functions on a topology" $ do
    ...
    ( if (nWrapped > nInner)
        then pure ()
          ...
          expectationFailure
            ( "expected dimap to wrap; unwrapped="
                <> show nInner
                <> " wrapped="
                <> show nWrapped
            )
...
test_suppressWindowedWith_compiles_with_max_records =
  it "maxRecordsBufferConfig / maxBytesBufferConfig configure BufferConfig" $ do
    ...
    let _ =
          F.suppressWindowedWith @Text @Int64
            (Duration 1000)
            1000
            (F.maxRecordsBufferConfig 100)
    pure ()
...
test_sourceWith_offset_reset_propagated_to_spec =
  it "Consumed.withOffsetResetPolicy lands on SourceSpec" $ do
    let cfg :: Consumed Text Text
        cfg =
          Consumed.withOffsetResetPolicy
            Consumed.OffsetLatest
