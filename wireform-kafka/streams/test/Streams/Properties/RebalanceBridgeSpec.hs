...
import Kafka.Client.ConsumerGroupV2 qualified as CGV2
import Kafka.Streams.Processor (TaskId (..))
import Kafka.Streams.Runtime.Assignor (MemberId (..))
import Kafka.Streams.Runtime.RebalanceBridge
import Kafka.Streams.Runtime.RebalanceProtocol qualified as RP
import Test.Syd


tests :: Spec
tests =
  describe "Rebalance bridge (KIP-848 \xe2\x86\x92 streams reconciler)" $
    sequence_
