...
tests :: TestTree
tests = testGroup "Rebalance bridge (KIP-848 \xe2\x86\x92 streams reconciler)"
...
unit_assigned_becomes_add :: TestTree
unit_assigned_becomes_add =
  testCase "adAssigned partitions become rAdd TaskIds" $ do
    let delta = mkDelta [("orders", 0), ("orders", 1)] [] []
        r = deltaToReconciliation tpToTask 0 delta
    RP.rAdd r @?= Set.fromList [TaskId 0 0, TaskId 0 1]
    RP.rRemove r @?= Set.empty

unit_revoked_becomes_remove :: TestTree
unit_revoked_becomes_remove =
  testCase "adRevoked partitions become rRemove TaskIds" $ do
    let delta = mkDelta [] [("orders", 2)] []
        r = deltaToReconciliation tpToTask 0 delta
    RP.rRemove r @?= Set.singleton (TaskId 0 2)

unit_lost_becomes_remove :: TestTree
unit_lost_becomes_remove =
  testCase "adLost partitions also flow into rRemove" $ do
    let delta = mkDelta [] [] [("orders", 3)]
        r = deltaToReconciliation tpToTask 0 delta
    RP.rRemove r @?= Set.singleton (TaskId 0 3)

----------------------------------------------------------------------
-- Round-trip through GroupState
----------------------------------------------------------------------

unit_applyDelta_updates_gsOwned :: TestTree
unit_applyDelta_updates_gsOwned =
  testCase "applyAssignmentDelta updates gsOwned for the local member" $ do
    let mid   = MemberId "m1"
    ...
    Map.findWithDefault Set.empty mid (RP.gsOwned gs1)
      @?= Set.fromList [TaskId 0 0, TaskId 0 1]
    let gs2 = applyAssignmentDelta tpToTask 0 mid delta2 gs1
    Map.findWithDefault Set.empty mid (RP.gsOwned gs2)
      @?= Set.fromList [TaskId 0 1, TaskId 0 2]

----------------------------------------------------------------------
-- End-to-end: planHeartbeat output drives the reconciliation
----------------------------------------------------------------------

unit_heartbeatPlan_drives_reconciliation :: TestTree
unit_heartbeatPlan_drives_reconciliation =
  testCase "planHeartbeat -> AssignmentDelta -> Reconciliation matches manual diff" $ do
    ...
    RP.rAdd r    @?= Set.singleton (TaskId 0 2)
    RP.rRemove r @?= Set.singleton (TaskId 0 0)
