...
data StepOutcome
  = StepOK
  | -- | callback returns @Left ...@
    StepLeft
  | -- | flush body throws (only valid for flush step)
    StepThrow
...
outcomesMatch e a = case (e, a) of
  ...
  (CommitAborted re, CommitAborted ra)
    ...
    | otherwise -> do
        H.annotate
          ( "aborted reason prefix mismatch: expected `"
              <> T.unpack re
              <> "`, got `"
              <> T.unpack ra
              <> "`"
  ...
  (CommitFatal re, CommitFatal ra)
    ...
    | otherwise -> do
        H.annotate
          ( "fatal reason prefix mismatch: expected `"
              <> T.unpack re
              <> "`, got `"
              <> T.unpack ra
              <> "`"
  ...
  where
    prefix pe pa =
      T.isPrefixOf (T.takeWhile (/= ' ') pe) pa
