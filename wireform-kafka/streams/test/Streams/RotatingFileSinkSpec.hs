...
import Kafka.Streams.Printed qualified as Printed
...
import Kafka.Streams.Printed qualified as RF
...
tests :: Spec
tests =
  describe "Kafka.Streams.Printed (rotating-file sink)" $
    sequence_
      ...
      bracket (RF.openRotatingHandle cfg) RF.closeRotatingHandle $ \rh -> do
        ...
        replicateM_ 10 $
          pipeInput
            driver
            (topicName "in")
            (Just "kkkkkkkkkkkkkkkk")
            (bytes "vvvvvvvvvvvvvvvv")
            (ts 0)
            0
        closeDriver driver
      ...
      Printed.withPrintedRotatingFile
        ...
        $ \printed -> do
          ...
          Printed.printKStream
            (Printed.withLabel "[from-printed]" printed)
            src
          topo <- buildTopology b
