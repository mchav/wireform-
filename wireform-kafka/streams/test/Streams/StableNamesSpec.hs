...
import Kafka.Streams.Topology.StableNames qualified as N
import Test.Syd


tests :: Spec
tests =
  describe "Topology stable names (KIP-307)" $
    sequence_
      [ it
          "names use 10-digit zero-padded counters"
          pad10
      , it
          "two builds of the same operator sequence agree"
          same_sequence_same_names
      , it
          "different operator classes use independent counters"
          independent_counters
      , it
          "operatorPrefix uses the canonical Kafka strings"
          canonical_prefixes
      ]
...
independent_counters =
  ...
  N.generateNames [N.OpFilter, N.OpMap, N.OpFilter, N.OpMap]
    `shouldBe` [ "KSTREAM-FILTER-0000000000"
