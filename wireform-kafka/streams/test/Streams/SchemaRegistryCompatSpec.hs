...
import Kafka.Streams.Serde.SchemaRegistry.Http qualified as SRHttp
import Test.Syd


tests :: Spec
tests =
  describe "Kafka.Streams.Serde.SchemaRegistry.Compat" $
    sequence_
...
http_request_shapes =
  it "HTTP request builders emit the expected paths" $ do
    ...
    SRHttp.reqUrl testReq
      `shouldBe` "http://reg.example/compatibility/subjects/events-value/versions/latest"
...
cannedRequester
  :: SRHttp.HttpResponse
  -- ^ response for compat-mode
  -> SRHttp.HttpResponse
  -- ^ response for test-compat
