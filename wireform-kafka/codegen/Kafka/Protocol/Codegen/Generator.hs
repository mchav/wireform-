...
import Kafka.Protocol.Codegen.WireGenerator qualified as WG
  ...
  in
    let
      ...
      perStructWire =
        ...
        , Just fns <-
            [ WG.generateNestedWireFunctions
                flexibleVersion
                structName
                structFields
            ]
  ...
  where
    ...
    generateField field =
      ...
      in vsep
           ...
           , fieldVersionInfo
           , indent 2 $
               pretty (toHaskellFieldName typeName (fieldName field))
                 <+> ":: !"
  ...
  let v0 = if 0 `elem` versions then [0] else []
      ...
      intermediate =
        if length versions > 4
          then
            let step = max 1 (length versions `div` 4)
