...
tests :: TestTree
tests = testGroup "HPACK edge cases"
  [ testGroup "Huffman edge cases"
      [ testCase "single character" $ do
          let encoded = huffmanEncode "a"
          huffmanDecode encoded @?= Right "a"
      , testCase "long repetitive input" $ do
          let input = BS.replicate 1024 0x61
              encoded = huffmanEncode input
          huffmanDecode encoded @?= Right input
      , testCase "all printable ASCII" $ do
          let input = BS.pack [32..126]
              encoded = huffmanEncode input
          huffmanDecode encoded @?= Right input
      , testProperty "roundtrip printable ASCII strings" $ \(ASCIIString s) ->
          not (null s) ==>
            let input = BS.pack (map (fromIntegral . fromEnum) s)
                encoded = huffmanEncode input
            in huffmanDecode encoded == Right input
      ]
  , testGroup "Header block edge cases"
      [ testCase "empty header list" $ do
          ...
          result @?= Right []
      , testCase "header with empty value" $ do
          encDt <- newDynamicTable 4096
          decDt <- newDynamicTable 4096
      ...
      , testCase "header with long value (4 KiB)" $ do
          ...
          result <- decodeHeaderBlock decDt encoded
          result @?= Right headers
      , testCase "many headers saturate dynamic table" $ do
          ...
          result <- decodeHeaderBlock decDt encoded
          result @?= Right headers
      , testCase "sequential encodes share dynamic table state" $ do
          encDt <- newDynamicTable 4096
          decDt <- newDynamicTable 4096
          ...
          r2 @?= Right h2
          assertBool "second encoding uses index → shorter"
            (BS.length e2 <= BS.length e1 + 3)
      , testCase "pseudo-headers come first" $ do
          encDt <- newDynamicTable 4096
          decDt <- newDynamicTable 4096
          let headers =
                [ (":method", "GET")
  ...
  , testGroup "Dynamic table sizing"
      [ testCase "zero-size table still works (no indexing)" $ do
