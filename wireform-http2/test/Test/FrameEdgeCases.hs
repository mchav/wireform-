...
tests :: TestTree
tests = testGroup "Frame edge cases"
  [ testGroup "payload roundtrips"
      [ testCase "PRIORITY frame" $ do
  ...
  , testGroup "error codes"
      ...
      , testCase "GOAWAY with empty debug data" $ do
  ...
  , testGroup "full frame encode/decode"
      [ testProperty "arbitrary DATA frames roundtrip" $ \bs ->
          let body = BS.pack bs
              frame = Frame
      ...
      , testProperty "WINDOW_UPDATE values roundtrip" $ \w ->
