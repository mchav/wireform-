...
tests :: TestTree
tests = testGroup "Connection"
  [ testGroup "Settings"
      [ testCase "apply valid settings" $ do
          let params = [(0x1, 8192), (0x3, 200), (0x4, 131072), (0x5, 32768)]
          case applySettingsParams defaultSettings params of
            Left err -> assertFailure (show err)
            Right s -> do
              settingsHeaderTableSize s @?= 8192
              settingsMaxConcurrentStreams s @?= Just 200
              settingsInitialWindowSize s @?= 131072
              settingsMaxFrameSize s @?= 32768
      , testCase "reject invalid max frame size (too small)" $ do
          let params = [(0x5, 100)]
          case applySettingsParams defaultSettings params of
            Left (InvalidMaxFrameSize _) -> pure ()
            _ -> assertFailure "Expected InvalidMaxFrameSize"
      , testCase "reject invalid max frame size (too large)" $ do
          let params = [(0x5, 20000000)]
          case applySettingsParams defaultSettings params of
            Left (InvalidMaxFrameSize _) -> pure ()
            _ -> assertFailure "Expected InvalidMaxFrameSize"
      , testCase "reject invalid initial window size" $ do
          let params = [(0x4, 0x80000000)]
          case applySettingsParams defaultSettings params of
            Left (InvalidInitialWindowSize _) -> pure ()
            _ -> assertFailure "Expected InvalidInitialWindowSize"
      , testCase "reject invalid enable push" $ do
          let params = [(0x2, 2)]
          case applySettingsParams defaultSettings params of
            Left (InvalidEnablePush _) -> pure ()
            _ -> assertFailure "Expected InvalidEnablePush"
      , testCase "unknown settings ignored" $ do
          let params = [(0xFF, 42)]
          case applySettingsParams defaultSettings params of
            Left err -> assertFailure (show err)
            Right s -> s @?= defaultSettings
      ]
  , testGroup "FlowControl"
      [ testCase "consume and release" $ do
          fc <- atomically $ newFlowControl 65535
          consumed <- atomically $ consumeWindow fc 1000
          consumed @? "Should consume"
          avail <- atomically $ availableWindow fc
          avail @?= 64535
          result <- atomically $ releaseWindow fc 1000
          case result of
            Right () -> pure ()
            Left _ -> assertFailure "Should not overflow"
          avail2 <- atomically $ availableWindow fc
          avail2 @?= 65535
      , testCase "refuse over-consume" $ do
          fc <- atomically $ newFlowControl 100
          consumed <- atomically $ consumeWindow fc 200
          not consumed @? "Should refuse"
      , testCase "detect overflow" $ do
          fc <- atomically $ newFlowControl 2147483647
          result <- atomically $ releaseWindow fc 1
          case result of
            Left _ -> pure ()
            Right () -> assertFailure "Should overflow"
      ]
  , testGroup "Client/Server handshake"
      [ testCase "basic preface exchange" $ do
