...
thriftToJSON = \case
  ...
  TV.Struct fields ->
    Aeson.object
      [ (Key.fromText (Text.pack (show fid)), thriftToJSON v)
...
thriftFromJSON template json = case template of
  ...
  TV.UUID _ -> case json of
    Aeson.String s -> case Base16.decode (TE.encodeUtf8 s) of
      Right bs
        | BS.length bs == 16 -> Right (TV.UUID bs)
...
encodeTypedStruct fields =
  Aeson.object
    [ (Key.fromText (Text.pack (show fid)), wrapTyped v)
    | (fid, v) <- V.toList fields
    ]
  ...
  TV.Map kt vt entries ->
    ...
    in Aeson.Array
         ( V.fromList
             ...
             , Aeson.object
                 [ (Key.fromText (stringifyKey k), rawTyped v)
...
isTypeTag t =
  t
    `elem` ["tf", "i8", "i16", "i32", "i64", "dbl", "str", "bin", "rec", "map", "lst", "set", "uuid"]
