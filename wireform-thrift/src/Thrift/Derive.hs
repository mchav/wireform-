  ...
  if miSkip mi
    then case miDefaults mi of
      ...
      Nothing ->
        [|
          Left
            ( "Thrift.Derive: missing 'defaults' for skipped field "
      ...
      base <-
        if isOptional
          then
            [|
              case lookupThriftField $(litE (integerL (fromIntegral fid))) $(varE kvs) of
            ...
            [|
              case lookupThriftField $(litE (integerL (fromIntegral fid))) $(varE kvs) of
                Nothing ->
                  Left
                    ( "Thrift.Derive: missing field id "
