...
sumCtorToMP c = do
  ...
  contentsE <- case fieldNames of
    ...
    ns   -> [| MV.Array (V.fromList
                $(pure (ListE (map (AppE (VarE 'M.toMsgPack) . VarE) ns)))) |]
  body <- [| MV.Map (V.fromList
...
fieldParser kvs (FieldInfo mSel _) = do
  ...
  if miSkip mi
    then case miDefaults mi of
      ...
      Nothing    -> [| Left ("MsgPack.Derive: missing 'defaults' for skipped field " ++
      ...
      base <-
        if isOptional
          then [| case lookupMPField $(pure keyExp) $(varE kvs) of
                    Nothing -> Right Nothing
                    Just  v -> fmap Just (M.fromMsgPack v) |]
          else [| case lookupMPField $(pure keyExp) $(varE kvs) of
                    Nothing -> Left ("MsgPack.Derive: missing field "
...
lookupMPField name kvs = V.foldr step Nothing kvs
  where
    step (MV.String k, v) acc | k == name = Just v
                              | otherwise = acc
    step _ acc = acc
  ...
  [| case $(varE cVar) of
       MV.Array $(varP arr)
         | V.length $(varE arr) == $(litE (integerL (fromIntegral arity)))
             -> $(pure body)
         | otherwise
             -> Left ("MsgPack.Derive: " ++ conNameStr
                      ++ " expected " ++ show arity ++ " contents, got "
                      ++ show (V.length $(varE arr)))
       _ -> Left ("MsgPack.Derive: " ++ conNameStr
