...
approach. An 'Encoding' is a 'Wireform.Builder.Builder' that,
...
lazyString !t =
  ...
  in Encoding (strHeader len <> foldMap BB.byteString (BSL.toChunks bs))
...
lazyBinary !bs =
  ...
  in Encoding (binHeader len <> foldMap BB.byteString (BSL.toChunks bs))
