...
-- using the Addr#-based fast decoder from "Proto.Decode.Fast" for minimal
-- GC overhead.
...
decodeArrayN p len off0 cnt origBs = go off0 cnt []
  where
    go !off 0 !acc = pure $ Right (MV.Array (V.fromList (reverse acc)), off)
    go !off n !acc = do
      r <- decodeValue p len off origBs
      case r of
        Left e       -> pure $ Left e
        Right (v, o) -> go o (n - 1) (v : acc)

decodeMapN :: Ptr Word8 -> Int -> Int -> Int -> ByteString -> DecResult
decodeMapN p len off0 cnt origBs = go off0 cnt []
  where
    go !off 0 !acc = pure $ Right (MV.Map (V.fromList (reverse acc)), off)
    go !off n !acc = do
