...
--
-- Reads a wire-format 'ByteString' and produces a 'ThriftValue' tree.
...
decodeBinValue !tt !bs !off = case tt of
  ...
  TT_UUID
    ...
    | otherwise -> Just (TVUUID (BS.take 16 (BS.drop off bs)), off + 16)

  TT_STOP -> Nothing

decodeBinMapEntries :: ThriftType -> ThriftType -> Int -> ByteString -> Int
                    -> [(ThriftValue, ThriftValue)] -> Maybe (ThriftValue, Int)
decodeBinMapEntries kt vt !n bs !off !acc
  | n <= 0 = Just (TVMap kt vt (reverse acc), off)
...
decodeBinListEntries :: ThriftType -> Int -> ByteString -> Int
                     -> [ThriftValue] -> Maybe (ThriftValue, Int)
decodeBinListEntries et !n bs !off !acc
  | n <= 0 = Just (TVList et (reverse acc), off)
...
decodeBinSetEntries et !n bs !off !acc
  | n <= 0 = Just (TVSet et (reverse acc), off)
  | otherwise = case decodeBinValue et bs off of
      Nothing -> Nothing
...
decodeCompact :: ByteString -> Either String ThriftValue
decodeCompact bs = case decodeCompStruct bs 0 of
  Nothing        -> Left "decodeCompact: failed to decode struct"
  Just (v, _off) -> Right v

decodeCompStruct :: ByteString -> Int -> Maybe (ThriftValue, Int)
decodeCompStruct bs off = go off 0 []
  where
    go :: Int -> Int16 -> [(Int16, ThriftValue)] -> Maybe (ThriftValue, Int)
    go !o !lastFid !acc = case tCompDecodeFieldBegin bs o lastFid of
      Nothing -> Nothing
      Just (TT_STOP, _, o', _) -> Just (TVStruct (reverse acc), o')
      Just (tt, fid, o', boolVal) -> case tt of
        TT_BOOL -> go o' fid ((fid, TVBool boolVal) : acc)
        _ -> case decodeCompValue tt bs o' of
          Nothing -> Nothing
          Just (v, o'') -> go o'' fid ((fid, v) : acc)

decodeCompValue :: ThriftType -> ByteString -> Int -> Maybe (ThriftValue, Int)
