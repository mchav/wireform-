  ...
  | otherwise =
      ...
      in if ver /= compactVersion
         then Left $ "decodeMessageCompact: unsupported version: " ++ show ver
                ...
                Just !mtype -> do
                  (seqidW, off1) <- maybeToEither "decodeMessageCompact: failed to read seqid varint"
                                      (getVarint bs 2)
                  let !seqid = fromIntegral seqidW :: Int32
                  (nameLen, off2) <- maybeToEither "decodeMessageCompact: failed to read name length varint"
                                      (getVarint bs off1)
