...
decodeAvro :: AvroType -> ByteString -> Either String AvroValue
...
decodeValue :: AvroType -> ByteString -> Int -> AvroDecodeResult AvroValue
...
decodeValue (AvroEnum {}) bs off =
  case avroDecodeInt bs off of
    AvroDecodeOK n off' -> AvroDecodeOK (AvEnum (fromIntegral n)) off'
    ...
    AvroDecodeOK idxI64 off' ->
      ...
      in if idx < 0 || idx >= V.length branches
         then AvroDecodeFail $ "Avro.Decode: union index out of range: " ++ show idx
           ...
           AvroDecodeOK val off'' -> AvroDecodeOK (AvUnion idx val) off''
           AvroDecodeFail e       -> AvroDecodeFail e
    AvroDecodeFail e -> AvroDecodeFail e
decodeValue (AvroFixed {avroFixedSize = sz}) bs off
  ...
  | otherwise = AvroDecodeOK (AvFixed (BS.take sz (BS.drop off bs))) (off + sz)
decodeValue (AvroLogical {avroLogicalBase = base}) bs off =
  decodeValue base bs off

decodePrimitive :: AvroSchema -> ByteString -> Int -> AvroDecodeResult AvroValue
decodePrimitive AvroNull bs off =
  case avroDecodeNull bs off of
    AvroDecodeOK _ off' -> AvroDecodeOK AvNull off'
    AvroDecodeFail e    -> AvroDecodeFail e
decodePrimitive AvroBool bs off =
  case avroDecodeBool bs off of
    AvroDecodeOK b off' -> AvroDecodeOK (AvBool b) off'
    AvroDecodeFail e    -> AvroDecodeFail e
decodePrimitive AvroInt bs off =
  case avroDecodeInt bs off of
    AvroDecodeOK n off' -> AvroDecodeOK (AvInt n) off'
    AvroDecodeFail e    -> AvroDecodeFail e
decodePrimitive AvroLong bs off =
  case avroDecodeLong bs off of
    AvroDecodeOK n off' -> AvroDecodeOK (AvLong n) off'
    AvroDecodeFail e    -> AvroDecodeFail e
decodePrimitive AvroFloat bs off =
  case avroDecodeFloat bs off of
    AvroDecodeOK f off' -> AvroDecodeOK (AvFloat f) off'
    AvroDecodeFail e    -> AvroDecodeFail e
decodePrimitive AvroDouble bs off =
  case avroDecodeDouble bs off of
    AvroDecodeOK d off' -> AvroDecodeOK (AvDouble d) off'
    AvroDecodeFail e    -> AvroDecodeFail e
decodePrimitive AvroBytes bs off =
  case avroDecodeBytes bs off of
    AvroDecodeOK b off' -> AvroDecodeOK (AvBytes b) off'
    AvroDecodeFail e    -> AvroDecodeFail e
decodePrimitive AvroString bs off =
  case avroDecodeString bs off of
    AvroDecodeOK t off' -> AvroDecodeOK (AvString t) off'
    AvroDecodeFail e    -> AvroDecodeFail e
decodePrimitive s _ _ = AvroDecodeFail $ "Avro.Decode: unsupported primitive: " ++ show s

decodeRecord :: [AvroField] -> ByteString -> Int -> [AvroValue] -> AvroDecodeResult AvroValue
decodeRecord [] _bs off acc = AvroDecodeOK (AvRecord (reverse acc)) off
...
decodeArray :: AvroType -> ByteString -> Int -> AvroDecodeResult AvroValue
decodeArray itemTy bs off0 = decodeArrayBlocks bs off0 []
  where
    decodeArrayBlocks :: ByteString -> Int -> [AvroValue] -> AvroDecodeResult AvroValue
    decodeArrayBlocks bsB off acc =
      case avroDecodeLong bsB off of
        AvroDecodeOK cnt64 off' ->
          let !cnt = cnt64
          in if cnt == 0
             then AvroDecodeOK (AvArray (reverse acc)) off'
             else if cnt < 0
    ...
    decodeArrayItems :: ByteString -> Int -> Int -> [AvroValue] -> AvroDecodeResult AvroValue
...
decodeMap :: AvroType -> ByteString -> Int -> AvroDecodeResult AvroValue
decodeMap valTy bs off0 = decodeMapBlocks bs off0 []
  where
    decodeMapBlocks :: ByteString -> Int -> [(Text, AvroValue)] -> AvroDecodeResult AvroValue
    decodeMapBlocks bsB off acc =
      case avroDecodeLong bsB off of
        AvroDecodeOK cnt64 off' ->
          let !cnt = cnt64
          in if cnt == 0
             then AvroDecodeOK (AvMap (reverse acc)) off'
             else if cnt < 0
             then case avroDecodeLong bsB off' of
               AvroDecodeOK _blockSize off'' ->
    ...
    decodeMapEntries :: ByteString -> Int -> Int -> [(Text, AvroValue)] -> AvroDecodeResult AvroValue
