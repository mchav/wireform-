...
fileDescriptorToAST fdp = ProtoFile
  ...
  , protoOptions   = []
  , protoTopLevels = fmap (TLMessage . descriptorToMessage) (V.toList (fdpMessageType fdp))
...
descriptorToMessage dp = MessageDef
  { msgName     = dpName dp
  ...
  where
    ...
    enumElems = fmap (MEEnum . enumDescriptorToEnum) (V.toList (dpEnumType dp))
...
partitionOneofFields oneofNames fields =
  let regular = filter (\f -> fdpFieldOneofIdx f < 0) fields
      grouped = fmap (\(i, _) -> filter (\f -> fdpFieldOneofIdx f == fromIntegral i) fields)
...
buildOneofDefs names groups = zipWith mkOneof names groups
  where
    mkOneof name flds = MEOneof (OneofDef
      { oneofName = name
      , oneofFields = fmap mkOneofField flds
      , oneofOptions = []
      })
    mkOneofField f = OneofField
...
fieldDescriptorToField f = FieldDef
...
enumDescriptorToEnum e = EnumDef
  { enumName    = edpName e
  , enumValues  = fmap (\v -> EnumValue (evdpName v) (fromIntegral (evdpNumber v)) [])
                       (V.toList (edpValue e))
  , enumOptions = []
  }
...
serviceDescriptorToService s = ServiceDef
  { svcName    = sdpName s
  , svcRpcs    = fmap methodToRpc (V.toList (sdpMethod s))
  , svcOptions = []
  }
  where
    methodToRpc m = RpcDef
      { rpcName      = mdpName m
