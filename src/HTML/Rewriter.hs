...
import HTML.Parse (Token(..), tokenizeBS)
...
matchAtPosition (ComplexSelector hd tl) st tag attrs =
  matchCompound hd tag attrs && matchTail tl (asStack st)
  where
    matchTail [] _ = True
    matchTail ((Descendant, comp):rest) frames =
      any (\(StackFrame t a _) -> matchCompound comp t a && matchTail rest (dropToFrame t frames)) frames
    matchTail ((Child, comp):rest) frames =
      case frames of
        (StackFrame t a _ : rest') -> matchCompound comp t a && matchTail rest rest'
        [] -> False
    matchTail ((AdjacentSibling, _):_) _ = False
    matchTail ((GeneralSibling, _):_) _ = False

    dropToFrame _ [] = []
    dropToFrame t (f:fs) = if sfTag f == t then fs else dropToFrame t fs
...
data RewriterState = RewriterState
  ...
  , rsBuf      :: !(IORef BB.Builder)
  , rsChunkBuf :: !(IORef BB.Builder)
  }

-- | One-shot: rewrite a complete document.
rewrite :: Rewriter -> ByteString -> IO ByteString
rewrite rw bs = do
  let !tokens = tokenizeBS bs 0 (BS.length bs) 0 False
  outRef <- newIORef mempty
  stRef <- newIORef newAutoState
  processTokens rw stRef outRef tokens
  -- Emit any remaining state
...
newRewriterState rw = do
  ...
  buf <- newIORef mempty
  chunk <- newIORef mempty
  pure (RewriterState rw auto buf chunk)

-- | Feed a chunk. Returns rewritten output for fully processed tokens.
feedRewriter :: RewriterState -> ByteString -> IO BB.Builder
feedRewriter rs chunk = do
  modifyIORef' (rsBuf rs) (<> BB.byteString chunk)
  pure mempty

-- | Finalize. Flushes all buffered content.
finishRewriter :: RewriterState -> IO BB.Builder
finishRewriter rs = do
  builder <- readIORef (rsBuf rs)
  let !bs = BL.toStrict (BB.toLazyByteString builder)
      !tokens = tokenizeBS bs 0 (BS.length bs) 0 False
  outRef <- newIORef mempty
  processTokens (rsRewriter rs) (rsAuto rs) outRef tokens
  readIORef outRef

-- | Streaming with output callback.
feedRewriter' :: RewriterState -> ByteString -> (ByteString -> IO ()) -> IO ()
feedRewriter' rs chunk _sink = do
  modifyIORef' (rsBuf rs) (<> BB.byteString chunk)
  pure ()
...
processTokens :: Rewriter -> IORef AutoState -> IORef BB.Builder -> [Token] -> IO ()
processTokens rw stRef outRef = go
  where
    emit !b = modifyIORef' outRef (<> b)

    go [] = pure ()
    go (tok : rest) = do
      processOneToken rw stRef emit tok
      go rest

processOneToken :: Rewriter -> IORef AutoState -> (BB.Builder -> IO ()) -> Token -> IO ()
processOneToken rw stRef emit tok = do
  st <- readIORef stRef
  case asSuppressUntil st of
    ...
    Nothing -> case asRemoveChildrenUntil st of
      Just rcDepth -> case tok of
        TEndTag _ _ -> do
          ...
          if newD <= rcDepth
            then do
              ...
              handleEndTag rw stRef emit tok
            else writeIORef stRef st { asDepth = newD }
        TStartTag _ _ sc _ ->
          when (not sc && not (isVoidTag (tokenTag tok))) $
            writeIORef stRef st { asDepth = asDepth st + 1 }
        _ -> pure ()
      Nothing -> case tok of
        TStartTag name attrs selfClose _ -> handleStartTag rw stRef emit name attrs selfClose
        TEndTag _ _ -> handleEndTag rw stRef emit tok
        TString text -> handleText rw stRef emit text True
        TChar c -> handleText rw stRef emit (T.singleton c) True
        TComment text -> handleComment rw stRef emit text
        TDoctype name pub sys _ -> handleDoctype rw stRef emit name pub sys
        TEOF -> pure ()

handleStartTag :: Rewriter -> IORef AutoState -> (BB.Builder -> IO ()) -> Text -> SmallArray HTMLAttribute -> Bool -> IO ()
handleStartTag rw stRef emit name attrs selfClose = do
  st <- readIORef stRef
  ...
  if null matchingElement
    then do
      emit (emitStartTagRaw name attrs selfClose)
...
handleEndTag :: Rewriter -> IORef AutoState -> (BB.Builder -> IO ()) -> Token -> IO ()
handleEndTag rw stRef emit tok = do
  st <- readIORef stRef
  let !d = asDepth st
      !name = tokenTag tok
      !newD = d - 1
...
handleText rw stRef emit text isLast = do
  st <- readIORef stRef
  let !matchingText = collectTextHandlersFromStack rw st
  if null matchingText
    then emit (escapeTextBuilder text)
    else do
      ...
      case replaced of
        ...
        _ -> do
          ...
          emit bef >> emit (escapeTextBuilder content) >> emit aft
...
collectTextHandlersFromStack rw st = do
  ...
  case ch of
    CHText complexSels handler ->
      case asStack st of
        (StackFrame parentTag parentAttrs _ : parentStack) ->
          let parentSt = st { asStack = parentStack }
          in if any (\cs -> matchAtPosition cs parentSt parentTag parentAttrs) complexSels
             then [handler]
             else []
        [] -> []
    _ -> []
...
matchEndPosition (ComplexSelector hd tl) st name =
  matchCompoundForEnd hd name (currentAttrs st) && matchTail tl (asStack st)
  where
    currentAttrs s = case asStack s of
      (StackFrame _ a _ : _) -> a
      []                     -> mempty
    matchTail [] _ = True
    matchTail ((Descendant, comp):rest) frames =
      any (\(StackFrame t a _) -> matchCompound comp t a && matchTail rest (tailAfter t frames)) frames
    matchTail ((Child, comp):rest) frames =
      case frames of
        (StackFrame t a _ : rest') -> matchCompound comp t a && matchTail rest rest'
        [] -> False
    matchTail _ _ = False
    tailAfter _ [] = []
    tailAfter t (f:fs) = if sfTag f == t then fs else tailAfter t fs
  ...
  where
    ...
    scanClean !bs !start !off !len
      ...
      | otherwise =
          let !b = BS.index bs off
          in case b of
            0x3C -> BB.byteString (BS.take (off - start) (BS.drop start bs))
                  <> BB.byteString "&lt;" <> scanClean bs (off + 1) (off + 1) len
            0x3E -> BB.byteString (BS.take (off - start) (BS.drop start bs))
                  <> BB.byteString "&gt;" <> scanClean bs (off + 1) (off + 1) len
            0x26 -> BB.byteString (BS.take (off - start) (BS.drop start bs))
                  <> BB.byteString "&amp;" <> scanClean bs (off + 1) (off + 1) len
  ...
  where
    scanClean !bs !start !off !len
      ...
      | otherwise =
          let !b = BS.index bs off
          in case b of
            0x22 -> BB.byteString (BS.take (off - start) (BS.drop start bs))
                  <> BB.byteString "&quot;" <> scanClean bs (off + 1) (off + 1) len
            0x26 -> BB.byteString (BS.take (off - start) (BS.drop start bs))
                  <> BB.byteString "&amp;" <> scanClean bs (off + 1) (off + 1) len
            0x3C -> BB.byteString (BS.take (off - start) (BS.drop start bs))
...
emitStartTagRaw tag attrs selfClose =
  BB.char7 '<' <> BB.byteString (TE.encodeUtf8 tag) <> emitAttrsRaw attrs
...
emitStartTagFromList tag attrs selfClose =
  BB.char7 '<' <> BB.byteString (TE.encodeUtf8 tag) <> emitAttrsFromList attrs
  <> (if selfClose then BB.byteString " />" else BB.char7 '>')
...
emitOneAttr name val
  | T.null val = BB.char7 ' ' <> BB.byteString (TE.encodeUtf8 name)
  | otherwise  = BB.char7 ' ' <> BB.byteString (TE.encodeUtf8 name)
              <> BB.byteString "=\"" <> escapeAttrBuilder val <> BB.char7 '"'
