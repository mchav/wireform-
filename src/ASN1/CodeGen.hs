...
genSequenceDecl name components = T.unlines $
  [ "data " <> name <> " = " <> name ]
  <> case components of
    [] ->
...
genComponentField recName (ComponentType fieldName td isOptional) =
  let accessor = asnFieldAccessorName recName fieldName
      hsType = if isOptional || isOptionalType td
...
genChoiceDecl name components = T.unlines $
  [ "data " <> name ]
  <> case components of
    [] -> [ "  deriving stock (Show, Eq, Generic)" ]
    (c:cs) ->
...
genEnumeratedDecl name vals = T.unlines $
  [ "data " <> name ]
  <> case vals of
...
mkASN1RecordField recName (ComponentType fieldName td isOptional) = do
  ...
  hsTy <- if isOptional || isOptionalType td
          then do
