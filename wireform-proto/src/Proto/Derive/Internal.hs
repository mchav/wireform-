...
import Proto.Wire qualified as PWire
import Proto.Wire.Decode qualified as PWD
import Proto.Wire.Encode qualified as PWE
...
lookupAccByName pairs sel = case [acc | (pf, acc) <- pairs, pfSelector pf == sel] of
  ...
  [] ->
    error
      ( "Proto.Derive.Internal: no accumulator for "
          ++ show sel
      )
