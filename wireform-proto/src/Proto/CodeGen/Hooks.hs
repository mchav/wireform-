...
import Proto.IDL.Options.Custom (CustomOptionRegistry, emptyCustomOptionRegistry, extractExtensionOptions, registerCustomOption)
...
thOnMessageAttribute attrName f =
  mempty
    { thOnMessage = \ctx ->
        case lookupAttribute attrName (mhcOptions ctx) of
          Just val -> f val ctx
