...
-- | Round-trip tests for 'Proto.Derive': encode each fixture to bytes,
...
tests = testGroup "Proto.Derive"
  ...
  , testCase "messageTypeName matches Haskell type" $ do
      PM.messageTypeName (Proxy :: Proxy User)    @?= T.pack "User"
      PM.messageTypeName (Proxy :: Proxy Address) @?= T.pack "Address"

  , testCase "deriveProtoFromTranslated: round-trip preserves UserT" $ do
      let u = (defaultUserT :: UserT)
                { tuserId      = 99
  ...
  , testCase "deriveProtoFromTranslated: byte-identical to deriveProto" $ do
      let u = defaultUser
                ...
                , userOffset  = -98765
                , userPort    = 0xCAFEBABE
                , userAddr    = Just Address
                    { addrStreet = T.pack "1 Wireform Way"
                    , addrCity   = T.pack "Berlin"
                    , addrZip    = 10119
                    }
                }
          uT = (defaultUserT :: UserT)
                { tuserId      = userId u
  ...
  , testCase "deriveProtoFromTranslated: AddressT messageTypeName" $ do
      PM.messageTypeName (Proxy :: Proxy AddressT) @?= T.pack "AddressT"
      PM.messageTypeName (Proxy :: Proxy UserT)    @?= T.pack "UserT"
  ...
  , testCase "Painting (enum field): zero-valued enum is skipped" $ do
      ...
      assertBool "color tag should be absent for default enum"
        (16 `BS.notElem` bs)
  ...
  , testCase "Profile (oneof): later variant wins on the wire" $ do
      ...
      let pUrl  = Profile { profName = T.empty
                          , profAvatar = Just (AvatarUrl (T.pack "old")) }
          pSeed = Profile { profName = T.empty
  ...
  , testCase "regression: name-only RegInventory matches" $ do
      let pBridge = BridgeRegInventory
