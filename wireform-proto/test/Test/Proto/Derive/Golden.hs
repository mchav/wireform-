...
-- 'Proto.Derive.Internal' — so the assertion had degraded to
...
tests :: TestTree
tests = testGroup "Proto.Derive byte-exact golden vectors"
  ...
  , testCase "RegItem { regi_name = \"widget\", regi_count = 99 }" $ do
      ...
      PE.encodeMessage p @?= bytes
        ...
        , 0x77, 0x69, 0x64       -- "wid"
        , 0x67, 0x65, 0x74       -- "get"
        , 0x10, 0x63             -- field 2 (varint int32), 99
        ]
  ...
  , testCase "RegInventory { name = \"depot\", items = 3 entries }" $ do
      ...
      PE.encodeMessage p @?= bytes
        ...
        [ 0x0A, 0x05, 0x64, 0x65, 0x70, 0x6F, 0x74
        -- field 2 (item): tag, len, payload  -- "alpha" + 1
        , 0x12, 0x09
        , 0x0A, 0x05, 0x61, 0x6C, 0x70, 0x68, 0x61
        , 0x10, 0x01
        -- field 2 (item): tag, len, payload  -- "beta" + 2
        , 0x12, 0x08
        , 0x0A, 0x04, 0x62, 0x65, 0x74, 0x61
        , 0x10, 0x02
        -- field 2 (item): tag, len, payload  -- "gamma" + 3
        , 0x12, 0x09
        , 0x0A, 0x05, 0x67, 0x61, 0x6D, 0x6D, 0x61
        , 0x10, 0x03
        ]
  ]
