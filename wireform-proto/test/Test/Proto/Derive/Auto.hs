-- | Round-trip tests for the auto-detecting 'Proto.Derive.deriveProto'
...
tests :: TestTree
tests = testGroup "Proto.Derive auto-detection (analyseField)"
  [ testGroup "auto-detected enum (Maybe AutoColor)"
  ...
  , testGroup "auto-detected list-repeated string (FKRepeated RepList)"
      [ testCase "empty list round-trips" $ do
          let c = AutoCard 0 Nothing []
          PD.decodeMessage (PE.encodeMessage c) @?= Right c

      , testCase "preserves order" $ do
          let c = AutoCard 1 Nothing
                    [ T.pack "alpha", T.pack "beta", T.pack "gamma" ]
          PD.decodeMessage (PE.encodeMessage c) @?= Right c
      ]

  , testGroup "auto-detected map (FKMap MapKeyString)"
  ...
  , testGroup "auto-detected oneof (sum of tagged single-arg constructors)"
  ...
  , testGroup "auto-detected Vector Int32 — packed encoding by default"
      ...
      , testCase "decoder accepts unpacked encoding (proto3 spec)" $ do
          ...
          let unpacked = BS.pack
                [ 0x10, 0x01    -- field 2 varint, value 1
                , 0x10, 0x02    -- field 2 varint, value 2
                , 0x10, 0x03    -- field 2 varint, value 3
                ]
      ...
      , testCase "packed bytes have only one tag for the whole vector" $ do
          ...
          assertBool ("expected exactly one 0x12 tag, got " <> show (countOf 0x12 bs))
            (countOf 0x12 bs == 1)
      ]
  ]
