...
tests =
  describe
    "Proto.TH.Derive auto-detection (analyseField)" $ sequence_
    [ describe
        "auto-detected enum (Maybe AutoColor)" $ sequence_
    ...
    , describe
        "auto-detected list-repeated string (FKRepeated RepList)" $ sequence_
    ...
    , describe
        "auto-detected map (FKMap MapKeyString)" $ sequence_
    ...
    , describe
        "auto-detected oneof (sum of tagged single-arg constructors)" $ sequence_
    ...
    , describe
        "auto-detected Vector Int32 — packed encoding by default" $ sequence_
