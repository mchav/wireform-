...
import Iceberg.Delete qualified as ID
import Iceberg.Read qualified
import Iceberg.Types (DeleteFile (..), DeleteFileContent (..), FileFormat (..))
import Iceberg.Types qualified
                  ...
                  icebergDs =
                    V.map
                      ( \(ID.PositionDeleteRow p pos) ->
                          Iceberg.Types.PositionDelete p pos
                      )
                      deletes'
                  kept =
                    Iceberg.Read.applyPositionDeletes
                      icebergDs
                      dataFilePath
                      dataRows
            ...
            then pure () -- pyarrow absent on the build agent; skip.
              ...
              case ID.writeEqualityDeleteFile
                ...
                Right (bytes, _) -> do
                  ...
                  pyarrowAssert
                    "pyarrow recovers field_id=17 for 'id'"
