...
tests :: Spec
tests =
  describe "Parquet.Encryption properties" $
    sequence_
      [ it
          "GCM encrypt/decrypt round-trip with derived nonce"
          propGcmRoundTrip
      , it
          "GCM rejects flipped tag bits"
          propGcmTamperTag
      , it
          "GCM rejects flipped ciphertext bits"
          propGcmTamperCiphertext
      , it
          "GCM detects AAD mismatch"
          propGcmAadMismatch
      , it
          "CTR encrypt/decrypt round-trip"
          propCtrRoundTrip
      , it
          "framed module: writeFramed -> readFramed -> decrypt"
          propFramedRoundTrip
      , it
          "AAD suffix is exactly 15 bytes for any input"
          propAadSuffixLength
      ]
