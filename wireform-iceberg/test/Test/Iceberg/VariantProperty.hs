...
tests :: Spec
tests =
  describe "Iceberg.Variant property tests" $
    sequence_
      [ it
          "encode/decode round-trip on arbitrary primitives"
          propPrimRoundTrip
      , it
          "encode/decode round-trip on arbitrary recursive Variants"
          propVariantRoundTrip
      , it
          "JSON projection round-trip on JSON-equivalent subset"
          propJsonRoundTrip
      , it
          "decimal16 round-trips arbitrary signed 128-bit unscaled values"
          propDecimal16Range
      , it
          "object lex-sorted keys: encode is canonical"
          propObjectSortedCanonical
      , it
          "shred -> reconstruct round-trip on JSON-equivalent inputs"
