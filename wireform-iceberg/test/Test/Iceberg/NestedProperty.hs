...
tests :: Spec
tests =
  describe "Parquet.Nested properties" $
    sequence_
      [ it
          "shred: def/rep stream lengths agree per leaf"
          propStreamLengths
      , it
          "shred: nlValueCount == count(def == maxDef)"
          propValueCount
      , it
          "shred: max def/rep don't exceed schema bounds"
          propLevelBounds
      , it
          "shred: leaf count matches flattenSchema"
          propLeafCount
      ]
      ...
      V.mapM_
        ( \l -> do
            let !maxD = fromIntegral (PN.nlMaxDef l) :: Int32
                ...
                !maxObservedDef =
                  if VP.null (PN.nlDefLevels l)
                    then 0
                    else VP.maximum (PN.nlDefLevels l)
                !maxObservedRep =
                  if VP.null (PN.nlRepLevels l)
                    then 0
