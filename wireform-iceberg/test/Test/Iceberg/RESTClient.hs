...
import Iceberg.Catalog.REST.Client qualified as Client
import Test.Syd


tests :: Spec
tests =
  describe "Iceberg.Catalog.REST.Client" $
    sequence_
      [ it "AuthHeader values are constructible" $ do
          case Client.NoAuth of _ -> pure ()
          case Client.BearerToken "tok" of _ -> pure ()
          case Client.RawHeader "X-Foo" "bar" of _ -> pure () :: IO ()
      , it "CommitTableRequest round-trips through Aeson" $ do
          let req =
                CommitTableRequest
          ...
          case Aeson.eitherDecode (Aeson.encode req) of
            Right (req' :: CommitTableRequest) -> req' `shouldBe` req
