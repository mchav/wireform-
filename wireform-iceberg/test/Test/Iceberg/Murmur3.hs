...
tests :: TestTree
tests = testGroup "Iceberg.Murmur3"
  [ testCase "hash(\"\") == 0" $
      M.murmur3_32 "" @?= 0

  , testCase "Iceberg bucket(int 34) % 16 = 3" $
  ...
  , testCase "Iceberg bucket(string \"iceberg\") deterministic" $
      ...
      let result = M.bucketBytes 16 (SV.encodeString (T.pack "iceberg"))
       in (result >= 0 && result < 16) @?= True

  , testCase "bucket result is always in [0, N)" $
