...
import Iceberg.Variant.Shredding qualified as Shred
...
tests :: Spec
tests =
  describe "Iceberg.Variant.Shredding" $
    sequence_
      ...
      , it "buildShreddedVariantParquetFile: Int64 routing" $ do
          let
            -- shared metadata: empty dictionary (the spec's smallest)
          ...
          case Shred.buildShreddedVariantParquetFile
            "measurement"
            meta
            Shred.ShredInt64
            rows of
      ...
      , it "reconstructVariant: partially-shredded object surfaces as Left" $ do
          let v = IV.VString "x"
              (m, valBytes) = IV.encodeVariant v
          case Shred.reconstructVariant
            m
            ( Shred.ShreddedColumn
                (Just valBytes)
                (Just (Shred.TVInt32 7))
            ) of
            Left _ -> pure ()
      ...
      , it "shred -> reconstruct round-trip preserves the Variant" $ do
          ...
          flip mapM_ cases $ \(v, st, expected) ->
            ...
            in case Shred.reconstructVariant meta sc of
                 ...
                 other ->
                   expectationFailure
                     ( "shred->reconstruct mismatch for "
                         ++ show v
                         ++ ": "
                         ++ show other
                     )
            ...
            then pure ()
              ...
              case Shred.buildShreddedVariantParquetFile
                ...
                Right bytes -> do
                  ...
                  pyarrowAssert
                    "shredded Variant file exposes the 3 columns"
