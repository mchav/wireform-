...
import qualified PureRef as Pure
...
main = defaultMain
  ...
  , bgroup "Roaring ARRAY container (always C-backed)"
      [ env (pure (VS.fromList [fromIntegral i :: Word16 | i <- [(0 :: Int), 2 .. 2000]])) $ \lows ->
          ...
          , let payload = Hash.roaringEncodeArray lows
            in bench "decode"
                  $ nf (Hash.roaringDecodeArray payload (VS.length lows))
                       (0 :: Word32)
          , let payload = Hash.roaringEncodeArray lows
            in bench "contains hit"
                  $ whnf (\v -> Hash.roaringContains Hash.ArrayContainer payload (VS.length lows) v)
                         (1000 :: Word16)
          , let payload = Hash.roaringEncodeArray lows
  ...
  , bgroup "Roaring BITSET container (always C-backed)"
      [ env (pure (VS.fromList [fromIntegral i :: Word16 | i <- [(0 :: Int) , 1 .. 30000]])) $ \lows ->
          ...
          , let payload = Hash.roaringEncodeBitset lows
            in bench "contains hit"
                $ whnf (\v -> Hash.roaringContains Hash.BitsetContainer payload 0 v)
                       (12345 :: Word16)
          ]
      ]
  ]
