...
writePositionDeleteFile
  :: Text
  -- ^ Output object-store path (recorded on the DeleteFile).
...
writeEqualityDeleteFile
  ...
  -> [EqualityDeleteSchema]
  -- ^ Column descriptors.
  -> V.Vector PW.ColumnData
  -- ^ Per-column data, same order.
  ...
  | otherwise = do
      let !rootElem =
            P.SchemaElement
              ...
              Nothing
              Nothing
              (Just (fromIntegral (length schemaCols)))
              Nothing
              Nothing
              Nothing
