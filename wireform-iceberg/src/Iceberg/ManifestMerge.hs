...
data MergePolicy = MergePolicy
  ...
  , mpTargetSizeBytes :: !Int64
  -- ^ @commit.manifest.target-size-bytes@ (default @8 MiB@)
  , mpMinCountToMerge :: !Int
  -- ^ @commit.manifest.min-count-to-merge@ (default @100@)
  , mpMaxFilesPerManifest :: !Int
  -- ^ Hard cap on entries per manifest written (default @8192@)
  }
  deriving (Show, Eq)
...
mergePolicyFromProperties props =
  MergePolicy
    { mpMergeEnabled =
        lookupBool
    ...
    , mpMaxFilesPerManifest =
        lookupInt
          "commit.manifest.max-files-per-manifest"
...
planFastAppend
  :: Int64
  -- ^ Sequence number this commit will receive.
  -> Int64
  -- ^ Snapshot id this commit will receive.
  -> Int
  -- ^ Partition spec id under which the new files were written.
  -> Text
  -- ^ Path the caller will use for the new manifest.
  -> Vector ManifestEntry
  -- ^ New manifest entries (typically all status=Added).
  -> Vector ManifestFile
  -- ^ Existing manifest list entries (carried through).
  -> CommitPlan
...
planAppend
  :: MergePolicy
  -> (Text -> Int -> Text)
  -- ^ Generator for new manifest paths: @gen seqNum index -> path@.
  ...
  -> Vector ManifestFile
  -- ^ Existing manifest list entries.
...
planAppend policy genPath readEntries seqNum snap specId newEntries existing
  ...
  | otherwise = do
      let (sameSpec, otherSpec) =
            V.partition
              ( \mf ->
                  mfPartitionSpecId mf == specId
...
planRewriteManifests
  ...
  -> (ManifestFile -> Either String (Vector ManifestEntry))
  -> Int64
  -- ^ Sequence number for this commit.
  -> Int64
  -- ^ Snapshot id for this commit.
  -> Int
  -- ^ Partition spec id.
  -> Vector ManifestFile
  -- ^ Manifests to rewrite.
  -> Vector ManifestFile
  -- ^ Manifests to keep untouched.
  -> Either String CommitPlan
...
assignSequenceNumbers seqNum snap me =
  me
...
newManifestTask
  ...
  -> Vector ManifestEntry
  -> WriteManifestTask
newManifestTask seqNum snap specId content path entries =
  WriteManifestTask
    { wmtPath = path
    , wmtSpecId = specId
    , wmtContent = content
...
manifestFilePlaceholder t =
  let entries = wmtEntries t
      ...
      sumRows status =
        V.foldl'
          ( \acc me ->
              if meStatus me == status
                then acc + meRecordCount me
                else acc
          )
          0
          entries
