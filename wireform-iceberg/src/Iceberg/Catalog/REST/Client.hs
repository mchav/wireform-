...
mkClientWithManager mgr base prefix auth =
  CatalogClient
...
viewPath cc ti =
  catalogPath
    cc
    ( "/namespaces/"
        <> namespaceSegment (REST.tiNamespace ti)
        <> "/views/"
        <> TE.encodeUtf8 (REST.tiName ti)
    )
...
tablePath cc ti =
  catalogPath
    cc
    ( "/namespaces/"
        <> namespaceSegment (tiNamespace ti)
        <> "/tables/"
        <> TE.encodeUtf8 (tiName ti)
    )
