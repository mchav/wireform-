...
createIcebergTablesDdl = T.unlines
...
createIcebergNamespacePropertiesDdl = T.unlines
...
data SqlCatalogError
  ...
  | CommitConflict !Text !Text  -- ^ The row's current
...
createTable
  :: SqlCatalog -> Vector Text -> Text -> Maybe Text
...
commitTable cat ns name assertedPrev newLoc = do
  affected <- case assertedPrev of
    Just prev -> sbExecuteCas (scBackend cat)
    ...
    Nothing -> sbExecuteCas (scBackend cat)
      "UPDATE iceberg_tables \
      \SET previous_metadata_location = metadata_location, \
      \    metadata_location = ? \
      \WHERE catalog_name = ? AND table_namespace = ? AND table_name = ? \
...
dropTable cat ns name = do
  affected <- sbExecute (scBackend cat)
...
renameTable
  :: SqlCatalog
  -> Vector Text -> Text -- ^ source ns + table
  -> Vector Text -> Text -- ^ target ns + table
  -> IO (Either SqlCatalogError ())
...
createNamespace
  :: SqlCatalog -> Vector Text -> Map Text Text
  -> IO (Either SqlCatalogError ())
...
updateNamespaceProperties
  :: SqlCatalog
  -> Vector Text       -- ^ namespace
  -> Vector Text       -- ^ properties to remove
  -> Map Text Text     -- ^ properties to insert / overwrite
  -> IO ()
updateNamespaceProperties cat ns removals updates = do
  mapM_
    (\k -> sbExecute (scBackend cat)
              "DELETE FROM iceberg_namespace_properties \
