  ...
  toJSON u = Aeson.Object $ KM.fromList ("action" Aeson..= action u : extraFields u)
    where
      ...
      extraFields = \case
        ...
        AddSchema s lastCol            -> [ ("schema", schemaToJSON s)
  ...
  toJSON r = Aeson.Object $ KM.fromList ("type" Aeson..= reqType r : reqFields r)
    where
      ...
      reqFields = \case
        ...
        AssertRefSnapshotId name sid         -> [ ("ref", Aeson.String name)
                                                , ("snapshot-id", maybe Aeson.Null
                                                    (Aeson.Number . fromIntegral) sid) ]
