...
-- doesn't exist yet.
currentMetadata
  :: HadoopCatalog -> V.Vector Text -> Text
...
commitMetadata
  :: HadoopCatalog
  -> V.Vector Text -- ^ namespace
  -> Text          -- ^ table name
  -> Maybe Int     -- ^ asserted current version, or 'Nothing' for create
  -> TableMetadata -- ^ the new metadata
  -> IO (Either String Int)
commitMetadata hc ns name asserted tm = do
  observed <- currentVersion hc ns name
  if observed /= asserted
    then pure (Left ("Hadoop catalog: version conflict, expected "
                     ++ show asserted ++ " but found " ++ show observed))
    else do
