...
import Iceberg.Expression qualified as Expr
import Iceberg.SchemaEvolution (currentSchema)
import Iceberg.SchemaEvolution qualified
import Iceberg.Snapshot (applicableDeletes, currentSnapshot)
import Iceberg.Snapshot qualified
...
readManifestEntriesWithInheritance
  :: ManifestFile
  -- ^ Owning manifest list entry, providing the inherited seq.
  -> ByteString
...
planScan tm manifestListBytes readManifestByPath = do
  snap <-
    maybe
      (Left "planScan: no current snapshot")
      Right
      (currentSnapshot tm)
  schema <-
    maybe
      (Left "planScan: no current schema")
      Right
      (currentSchema tm)
...
planScanWithDeletes tm manifestListBytes readManifestByPath = do
  snap <-
    maybe
      (Left "planScanWithDeletes: no current snapshot")
      Right
      (currentSnapshot tm)
  schema <-
    maybe
      (Left "planScanWithDeletes: no current schema")
      Right
      (currentSchema tm)
  (_, manifestFiles) <- readManifestList manifestListBytes
...
planScanWithFilter tm manifestListBytes readManifestByPath expr = do
  snap <-
    maybe
      (Left "planScanWithFilter: no current snapshot")
      Right
      (currentSnapshot tm)
  schema <-
    maybe
      (Left "planScanWithFilter: no current schema")
      Right
      (currentSchema tm)
...
data IncrementalScanPlan = IncrementalScanPlan
  { ispFromSnapshot :: !(Maybe Int64)
  -- ^ Exclusive lower bound; @Nothing@ = from beginning of history.
  , ispToSnapshot :: !Int64
  -- ^ Inclusive upper bound (the latest snapshot to include).
...
planIncrementalAppend tm fromSid toSid readMlByPath readManifestByPath = do
  toSnap <-
    maybe
      (Left $ "planIncrementalAppend: no snapshot " ++ show toSid)
      Right
...
collectSnapsBetween tm fromSid to0 = go to0
  where
    go s
      ...
      | otherwise =
          s : case snapParentId s >>= Iceberg.Snapshot.snapshotById tm of
...
planIncrementalChangelog tm fromSid toSid readMlByPath readManifestByPath = do
  toSnap <-
    maybe
      (Left $ "planIncrementalChangelog: no snapshot " ++ show toSid)
      Right
      (Iceberg.Snapshot.snapshotById tm toSid)
  let snapsInRange = collectSnapsBetween tm fromSid toSnap
      taskFor snap = do
