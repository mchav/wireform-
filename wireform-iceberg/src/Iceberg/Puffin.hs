...
writePuffin pf =
  let -- assign offsets to each blob
...
blobJSON b off len = Aeson.Object $ KM.fromList $
  ...
  ++ (if Map.null (pbProperties b)
        then []
        else [("properties", Aeson.Object $ KM.fromList
...
parseBlob fileBs (Aeson.Object obj) = do
  ...
  let codec = case KM.lookup "compression-codec" obj of
        ...
        _ -> Nothing
  Right PuffinBlob
    { pbType = ty, pbFields = fs, pbSnapshotId = sid, pbSequenceNumber = seqN
    , pbProperties = props, pbCompressionCodec = codec, pbData = blobBytes
    }
