...
import qualified Iceberg.BoundTrunc as BT
import qualified Iceberg.DeletionVector as DV
import qualified Iceberg.MetricsConfig as MC
...
fromParquetMetadata
  :: P.FileMetadata
  -> (Vector Text -> Maybe Int)   -- ^ Column path -> Iceberg field id.
  -> Map Text Text                -- ^ Table properties (used for metrics modes).
  -> Text                         -- ^ Data file path.
  -> Int64                        -- ^ Data file size (bytes on disk).
  -> Vector (Maybe Value)         -- ^ Partition tuple.
  -> Maybe Int                    -- ^ Sort order id (when applicable).
  -> DataFile
...
pagesOverlappingDeletes dv oi =
  let !pages = P.oiPageLocations oi
      ...
      hits = V.imapMaybe (\i (firstRow, nextFirstRow) ->
...
filterDeletedPages
  :: DV.DeletionVector
  -> P.OffsetIndex
  -> Int64           -- ^ Total row count of the column chunk.
  -> Vector Int
