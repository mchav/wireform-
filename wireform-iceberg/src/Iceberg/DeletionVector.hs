...
encodeDV (DeletionVector m) =
  let buckets = IntMap.toAscList m
      ...
      builder =
        ...
        <> mconcat [ BB.word32LE (fromIntegral hi) <> encodeRoaring32 set
                   | (hi, set) <- buckets ]
...
encodeRoaring32 set =
  let positions = IntSet.toAscList set
      ...
      keys = mconcat [ BB.word16LE (fromIntegral key)
      ...
      payload = mconcat [ mconcat (map (BB.word16LE . fromIntegral) lows)
                        | (_, lows) <- grouped ]
