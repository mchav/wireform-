...
checkFormatVersion tm
  ...
  | tmFormatVersion tm > 3 = ["unsupported format-version: "
...
checkSchemaList tm = concat
  [ [ "current-schema-id " <> T.pack (show (tmCurrentSchemaId tm))
      <> " not found in schemas"
    | not (any (\s -> schemaId s == tmCurrentSchemaId tm)
...
checkPartitionSpecs tm = concat
  [ [ "default-spec-id " <> T.pack (show (tmDefaultSpecId tm))
      <> " not found in partition-specs"
    | not (any (\s -> psSpecId s == tmDefaultSpecId tm)
...
checkSnapshotRefs tm = concat
  [ [ "snapshot ref \"" <> name
      <> "\" points at unknown snapshot " <> T.pack (show (srSnapshotId r))
...
checkIdentifierFields tm = concat
...
validateIdentifierFields schema =
  ...
  where
    check fid byId = case Map.lookup fid byId of
      Nothing -> Just $ "identifier-field-ids: no such top-level field id "
                        <> T.pack (show fid)
      Just sf
        | not (sfRequired sf) ->
            Just $ "identifier-field-id " <> T.pack (show fid)
                   <> " is on optional field \"" <> sfName sf <> "\""
