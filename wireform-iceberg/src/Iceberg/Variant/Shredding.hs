...
import Iceberg.Variant qualified as IV
...
data ShreddedRow
  ...
  | -- | re-encoded Variant value bytes
...
buildShreddedVariantParquetFile
  ...
  -- ^ column name
  -> ByteString
  -- ^ shared Variant metadata (used for every row)
  -> ShreddedType
  -> V.Vector (Maybe IV.Variant)
  -- ^ row-major Variant data
...
valueFor = \case
  ...
  ShredAsArrayOf _ -> PN.NRNull -- nested array; value lives in child columns
  ShredAsObjectOf _ -> PN.NRNull -- nested object; value lives in child columns
...
reconstructVariant meta sc = case (sc_value sc, sc_typedValue sc) of
  ...
  (Just _, Just _) ->
    ...
    Left
      "reconstructVariant: both value and typed_value present; \
...
routeObjectRow oss (Just (IV.VObject m)) =
  let !shreddedNames = map sfName (ossFields oss)
      ...
      !nonShredded =
        Map.filterWithKey
          (\k _ -> not (Map.member k shreddedSet))
          m
      !valueBytes
...
reconstructObjectVariant meta osr = case (osrValue osr, osrTypedFields osr) of
  ...
  (Just vBytes, Just typed) -> do
    ...
    decoded <- case IV.decodeVariant meta vBytes of
      ...
      Right other ->
        Left
          ( "reconstructObjectVariant: partially-shredded value "
    ...
    if not (Map.null overlap)
      then
        Left
          ( "reconstructObjectVariant: shredded and non-shredded "
  ...
  where
    ...
    go ((name, row) : rest) = do
      mv <- case row of
        ...
        ShredAsArrayOf inner ->
          reconstructArrayVariant meta inner
        ShredAsObjectOf inner ->
          reconstructObjectVariant meta inner
      rest' <- go rest
      case mv of
...
reconstructArrayVariant meta asr =
  case (asrValue asr, asrTypedElements asr) of
    ...
    (Just _, Just _) ->
      Left
        "reconstructArrayVariant: invalid encoding (both value \
  ...
  where
    reconstructElement m row = case row of
      ...
      ShredMissing ->
        ...
        Left
          "reconstructArrayVariant: missing element \
...
typedLeafType = \case
  ...
  ShredArrayOf _ -> error "typedLeafType: ShredArrayOf has no primitive leaf type"
  ShredObjectOf _ -> error "typedLeafType: ShredObjectOf has no primitive leaf type"
