...
icebergTypeToJSON (TGeography crs algo) =
  Aeson.String $
    T.concat ["geography(", crs, ", ", algo, ")"]
icebergTypeToJSON (TStruct fields) =
  Aeson.Object $
    KM.fromList
      [ ("type", Aeson.String "struct")
      , ("fields", Aeson.Array (V.map structFieldToJSON fields))
      ]
icebergTypeToJSON (TList elemId elemTy) =
  Aeson.Object $
    KM.fromList
...
icebergTypeToJSON (TMap keyId keyTy valId valTy) =
  Aeson.Object $
    KM.fromList
    ...
    Just (Aeson.Array xs) ->
      V.mapM
        ( \x -> case x of
            ...
            _ -> Left "partition field 'source-ids' entry not a number"
        )
        xs
...
partitionStatisticsFileToJSON p =
  Aeson.Object $
    KM.fromList
  ...
  Aeson.Object $
    KM.fromList $
      ...
      , ("representations", Aeson.Array (V.map viewRepresentationToJSON (vvRepresentations vv)))
        ...
        ++ ( if V.null (vvDefaultNamespace vv)
               then []
               else
...
viewRepresentationToJSON (SqlViewRepresentation sql dialect) =
  Aeson.Object $
    KM.fromList
...
viewRepresentationToJSON (UnknownViewRepresentation t fields) =
  Aeson.Object $
    KM.fromList $
      ("type", Aeson.String t)
