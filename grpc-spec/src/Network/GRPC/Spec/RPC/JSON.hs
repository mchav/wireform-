...
instance
  ...
  => SupportsClientRpc (JsonRpc serv meth)
  where
...
instance
  ...
  => SupportsServerRpc (JsonRpc serv meth)
  where
...
instance
  (Show x, Show (JsonObject fs))
  => Show (JsonObject ('(f, x) : fs))
  where
  showsPrec p (x :* xs) =
    showParen (p >= 6) $
      showsPrec 6 x
...
instance
  (NFData x, NFData (JsonObject fs))
  => NFData (JsonObject ('(f, x) : fs))
  where
  rnf (x :* xs) = rnf (x, xs)
...
instance
  (KnownSymbol f, ToJSON x, EncodeFields fs)
  => EncodeFields ('(f, Required x) : fs)
  where
...
instance
  (KnownSymbol f, FromJSON x, DecodeFields fs)
  => DecodeFields ('(f, Required x) : fs)
  where
  decodeFields obj = do
    fs <- decodeFields obj
