...
mkSF i n r t d =
  StructField
    ...
    , sfType = t
    , sfDoc = d
    , sfInitialDefault = Nothing
    , sfWriteDefault = Nothing
    }
...
mkSnap sid pid sn ts ml summ =
  Snapshot
    { snapId = sid
    , snapParentId = pid
    , snapSequenceNumber = sn
    , snapTimestampMs = ts
    , snapManifestList = ml
    , snapSummary = summ
              ...
              fields =
                V.singleton
                  ( mkSF
                      ...
                      "scores"
                      True
                      (TMap 100 TString 101 (TStruct valFields))
                      Nothing
                  )
              schema = mkSchema 0 fields
...
manifestPathFilterTests =
  describe "Manifest path filtering" $
    sequence_
...
sequenceNumberTests =
  describe "Sequence number filtering" $
    sequence_
...
scanPlanDeleteTests =
  describe "ScanPlan with deletes" $
    sequence_
...
mkManifestFile path content seqNum =
  ManifestFile
    { mfPath = path
    , mfLength = 1000
    , mfPartitionSpecId = 0
