...
msgPackTests :: Spec
msgPackTests = describe "MsgPack Encode/Decode" $ sequence_
  [ propertyRoundtrip
  , unitExactEncoding
  , unitEdgeCases
  , unitLargeContainers
...
propertyRoundtrip = describe "Roundtrip (property)" $ sequence_
  [ it "Nil" $ property $ do
      let v = MV.Nil
...
unitExactEncoding = describe "Exact byte encoding" $ sequence_
  [ it "nil = 0xc0" $
...
unitEdgeCases :: Spec
unitEdgeCases = describe "Edge cases" $ sequence_
...
unitLargeContainers = describe "Large containers (array16/map16)" $ sequence_
...
unitExtRoundtrip = describe "Ext type roundtrip" $ sequence_
  [ it "fixext1" $ do
...
unitTimestampRoundtrip = describe "Timestamp roundtrip" $ sequence_
...
jsonConversionTests = describe "JSON conversion" $ sequence_
  [ it "nil -> null -> nil" $ do
      let v = MV.Nil
