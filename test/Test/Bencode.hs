...
bencodeTests :: Spec
bencodeTests = describe "Bencode" $ sequence_
  [ valueTests
  , encodeTests
  , decodeTests
  , roundtripTests
...
valueTests :: Spec
valueTests = describe "Value types" $ sequence_
  ...
  , it "BList construction" $
      BList (V.fromList [BInteger 1, BInteger 2]) `shouldBe`
        BList (V.fromList [BInteger 1, BInteger 2])
  , it "BDict construction" $
      BDict (V.fromList [("key", BString "val")]) `shouldBe`
