  ...
  where
    purgePartition !s part fids =
      let !pm = fromMaybe Map.empty (Map.lookup part (tsPartitions s))
          !pm' = foldlStrict (flip Map.delete) pm fids
...
instance FromJSON HoodieCleanPartitionMetadata where
  parseJSON = withObject "HoodieCleanPartitionMetadata" $ \o -> do
    p <- o .:? "partitionPath"
    HoodieCleanPartitionMetadata (fromMaybe "" p)
      <$> o .:? "policy"
...
instance FromJSON HoodieCleanMetadata where
  parseJSON = withObject "HoodieCleanMetadata" $ \o ->
    HoodieCleanMetadata
...
-- A strict left fold used inside 'applyReplaceCommit'. Defined locally
-- under a unique name because Prelude only re-exports foldl' from GHC
-- 9.10 onward; sharing the name would be an ambiguous occurrence there.
