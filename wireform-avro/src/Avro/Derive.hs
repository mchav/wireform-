...
fieldCell vs pos (FieldInfo mSel _) = do
  ...
  if miSkip mi
    then case miDefaults mi of
      ...
      Nothing ->
        let msg =
              "Avro.Derive: missing 'defaults' for skipped field "
                ++ nameBase selName
      ...
      let posLit = litE (integerL (fromIntegral pos))
          base =
            [|
              if V.length $(varE vs) > $posLit
                then A.fromAvro ($(varE vs) V.! $posLit)
                else
                  Left
                    ( "Avro.Derive: record missing field at index "
                        ++ show ($posLit :: Int)
                    )
              |]
...
fromAvroSum cs = do
  ...
  lamE
    ...
    ( caseE
        (varE v)
        [ match
            (conP 'AV.Union [varP iVar, varP payload])
            (normalB (pure multi))
            []
        , match
            wildP
