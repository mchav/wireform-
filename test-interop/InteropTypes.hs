...
import Proto.Wire.Decode (runDecoder')
...
instance MessageEncode Repeated where
  buildMessage r =
    let ints = rInts r
        packedInts = if V.null ints then mempty
          else let payload = messageToByteString (V.foldl' (\acc v -> acc <> putVarint (fromIntegral v)) mempty ints)
