...
tests :: Spec
tests =
  describe "Python Interop Conformance" $
    sequence_
      [ describe "Haskell-encode → Python-decode → re-encode → Haskell-decode" $
          sequence_
      ...
      , describe "Python-encode → Haskell-decode" $
          sequence_
            [ it "Scalars with all fields" $ do
                bs <-
                  pythonEncode "Scalars" $
