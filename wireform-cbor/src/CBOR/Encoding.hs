...
--
-- An 'Encoding' is a 'Data.ByteString.Builder.Builder' that, when run,
...
-- | Render an 'Encoding' as a 'Data.ByteString.Builder.Builder'.
...
lazyBytes !bs = Encoding (header 0x40 (fromIntegral (BSL.length bs)) <> BB.lazyByteString bs)
...
lazyText !t =
  ...
  in Encoding (header 0x60 (fromIntegral (BSL.length bs)) <> BB.lazyByteString bs)
