...
sumCtorToCBOR c = do
  ...
  contentsE <- case fieldNames of
    []   -> [| CV.Null |]
    [n]  -> [| C.toCBOR $(varE n) |]
    ns   -> [| CV.Array (V.fromList
                $(pure (ListE (map (AppE (VarE 'C.toCBOR) . VarE) ns)))) |]
  body <- [| CV.Map (V.fromList
...
fieldParser kvs (FieldInfo mSel _) = do
  ...
  if miSkip mi
    then case miDefaults mi of
      ...
      Nothing    -> [| Left ("CBOR.Derive: missing 'defaults' for skipped field " ++
      ...
      base <-
        if isOptional
          then [| case lookupCBORField $(pure keyExp) $(varE kvs) of
                    Nothing -> Right Nothing
                    Just  v -> fmap Just (C.fromCBOR v) |]
          else [| case lookupCBORField $(pure keyExp) $(varE kvs) of
                    Nothing -> Left ("CBOR.Derive: missing field "
...
lookupCBORField name kvs = V.foldr step Nothing kvs
  where
    step (CV.TextString k, v) acc | k == name = Just v
                                  | otherwise = acc
    step _ acc = acc
...
fromCBOREnum cs = do
  ...
  lamE [varP v]
    (caseE (varE v)
       [ match (conP 'CV.TextString [varP s])
               (normalB (pure multi))
               []
       , match wildP
  ...
  [| case $(varE cVar) of
       CV.Array $(varP arr)
         | V.length $(varE arr) == $(litE (integerL (fromIntegral arity)))
             -> $(pure body)
         | otherwise
             -> Left ("CBOR.Derive: " ++ conNameStr
                      ++ " expected " ++ show arity ++ " contents, got "
                      ++ show (V.length $(varE arr)))
       _ -> Left ("CBOR.Derive: " ++ conNameStr
