...
import Network.GRPC.Spec
    ( toGrpcStatus, OrcaLoadReport
    , ProperTrailers_(..), ProperTrailers
    , customMetadataMapFromList, buildMetadataIO
    )
...
echoStatus :: Maybe EchoStatus -> IO ()
echoStatus Nothing = return ()
echoStatus (Just status) =
  ...
  where
    code :: Word
    code = fromIntegral $ (status ^. #code :: Int32)
...
-- | Send trailers with an optional ORCA load report attached.
--
-- When @mOrca@ is @Just@, the load report is serialized into the
-- @endpoint-load-metrics-bin@ trailer per the gRPC ORCA spec (A51).
sendTrailersWithOrca ::
     Call (Protobuf TestService rpc)
  -> InteropRespTrailMeta
  -> Maybe OrcaLoadReport
  -> IO ()
sendTrailersWithOrca call metadata mOrca = do
    metadata' <- customMetadataMapFromList <$> buildMetadataIO metadata
    let trailers :: ProperTrailers
        trailers = ProperTrailers {
              properTrailersGrpcStatus     = GrpcOk
            , properTrailersGrpcMessage    = Nothing
            , properTrailersStatusDetails  = Nothing
            , properTrailersPushback       = Nothing
            , properTrailersOrcaLoadReport = mOrca
