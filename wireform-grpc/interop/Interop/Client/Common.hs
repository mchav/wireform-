...
mkSimpleRequest expectCompressed =
    (mempty)
      & #expectCompressed .~ Just (getProto $ boolValue expectCompressed)
      & #responseSize     .~ 314159
      & #payload          .~ Just (getProto $ payloadOfZeroes 271828)
...
mkStreamingInputCallRequest expectCompressed size =
    (mempty)
      & #expectCompressed .~ Just (getProto $ boolValue expectCompressed)
      & #payload          .~ Just (getProto $ payloadOfZeroes size)
...
mkStreamingOutputCallRequest expectedSizes payload =
    (mempty)
      & #responseParameters .~ V.fromList
           [ (mempty)
               & #size       .~ fromIntegral sz
               & #compressed .~ Just (getProto $ boolValue compressed)
           | (compressed, sz) <- expectedSizes
           ]
      & ( case payload of
            Nothing -> id
            Just sz -> #payload .~ Just (getProto $ payloadOfZeroes sz)
        )
...
verifySimpleResponse resp = do
    let body = maybe mempty (^. #body) (resp ^. #payload)
    assertEqual 314159 $ BS.Strict.length body
    assertBool         $ BS.Strict.all (== 0) body
...
verifyStreamingOutputCallResponse expectedSize resp = do
    let body = maybe mempty (^. #body) (resp ^. #payload)
    assertEqual expectedSize $ BS.Strict.length body
    assertBool               $ BS.Strict.all (== 0) body
