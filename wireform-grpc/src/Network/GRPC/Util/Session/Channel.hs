...
import Control.Concurrent.STM.TMVar (TMVar, newEmptyTMVarIO, putTMVar, readTMVar, takeTMVar)
...
recv'
  ...
  -- ^ Message without trailers
  -> (Trailers (Inbound sess) -> b)
  -- ^ Trailers without (final) message
...
close
  ...
  => Channel sess
  -> ExitCase a
  -- ^ The reason why the channel is being closed
