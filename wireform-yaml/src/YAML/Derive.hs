  ...
  if miSkip mi
    then case miDefaults mi of
      ...
      Nothing ->
        [|
          Left
            ( "YAML.Derive: missing 'defaults' for skipped field "
      ...
      base <-
        if isOptional
          then
            [|
              case lookupYAMLField $(pure keyExp) $(varE kvs) of
