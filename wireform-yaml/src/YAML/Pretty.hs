...
import YAML.Annotated
  ( ABody (..), AMapEntry (..), AStream (..), AValue (..)
  , ADocument (..), Comment (..), MapStyle (..), ScalarStyle (..)
  , SeqStyle (..), SrcSpan (..), Trivia (..)
  )
import YAML.Value
  ( Anchor (..), Document (..), Stream (..), Tag (..), Value (..) )
...
renderStream opts (Stream docs)
  ...
  where
    step acc (i, d) =
      let !marker = if i == 0 && not (roDocStartMarker opts)
                      then mempty else fromText "---\n"
          !body   = build opts (YV.docBody d)
          !end    = if YV.docExplicitEnd d
                      then fromText "...\n"
                      else mempty
...
renderAnnotatedStream opts (AStream docs) msrc
  ...
  where
    step acc (i, d) =
      let marker = if i == 0 && not (adDirectivesEnd d)
                     then mempty else fromText "---\n"
          body   = fromText
...
build opts v0 =
  let body  = buildValue opts 0 ChildOfDoc v0
      body' = body <> (if roTrailingNewline opts && not (endsWithNL body)
                         then singleton '\n' else mempty)
...
buildSeqItem opts ind v
  ...
  , not (V.null kvs) = emitCompactSeqMap opts ind kvs
  | YSeq xs <- YV.unwrap v
  , not (V.null xs) =
...
shouldFlowMap opts ind kvs = \case
  MapFlow  -> True
  MapBlock ->
    ind + V.sum (V.map (\(k, v) -> approxFlowLength k + approxFlowLength v) kvs)
        + 4 * V.length kvs + 2
      <= roMaxLineWidth opts
...
needsQuoting _opts t
  ...
  | otherwise =
      ...
      in    trimmed == "null" || trimmed == "Null" || trimmed == "NULL"
         || trimmed == "true" || trimmed == "True" || trimmed == "TRUE"
         || trimmed == "false"|| trimmed == "False"|| trimmed == "FALSE"
         || trimmed == "~"
...
emitFlowSeq opts ind xs
  | V.null xs = fromText "[]"
  | otherwise =
      let approx = ind + 2 + V.length xs * 2
...
emitFlowMap opts ind kvs
  | V.null kvs = fromText "{}"
  | otherwise =
      let approx = ind + 2 + V.length kvs * 4
                       + V.sum (V.map (\(k, v) ->
                              approxFlowLength k + approxFlowLength v) kvs)
      in if approx <= roMaxLineWidth opts
           then singleton '{'
                <> mconcat (
...
sep :: (RenderOptions -> Value -> Builder)
    -> RenderOptions
    -> Int
    -> Value
    -> Builder
...
buildAValue
  :: RenderOptions -> Maybe Text -> Int -> ChildContext -> AValue
  -> Builder
...
buildABody opts msrc ind ctx av = case avBody av of
  ...
  ATagged tg inner ->
      contextSeparator ctx <> tagPrefix tg <> singleton ' '
  ...
  AAnchored (Anchor n) inner ->
      contextSeparator ctx
        ...
        <> buildAValue opts msrc ind ChildOfMapValueInline inner
  AAlias (Anchor n) ->
      contextSeparator ctx <> singleton '*' <> fromText n
        <> singleton '\n'

emitAString
  :: RenderOptions -> Int -> ChildContext -> Text -> ScalarStyle
  -> Builder
emitAString opts _ind ctx t style =
  contextSeparator ctx <> renderScalarStyled opts t style
    <> singleton '\n'
...
emitASeq
  :: RenderOptions -> Maybe Text -> Int -> ChildContext
  -> V.Vector AValue -> SeqStyle -> Builder
emitASeq opts _msrc ind ctx xs SeqFlow =
  contextSeparator ctx <> emitFlowSeq opts ind (V.map astripAnnotationsForFlow xs)
    <> singleton '\n'
...
emitACompactSeqMap
  :: RenderOptions -> Maybe Text -> Int -> V.Vector AMapEntry -> MapStyle
  -> Builder
...
emitAMap
  :: RenderOptions -> Maybe Text -> Int -> ChildContext
  -> V.Vector AMapEntry -> MapStyle -> Builder
emitAMap opts _msrc ind ctx es MapFlow =
  contextSeparator ctx <> emitFlowMap opts ind (V.map entryToPair es)
    <> singleton '\n'
  where
    entryToPair e = ( astripAnnotationsForFlow (amKey e)
    ...
    step !acc e =
      ...
      in case verbatimEntry opts msrc vAv of
           ...
           Nothing ->
             ...
             <> fromText ":"
             <> buildAValue opts msrc (ind + roIndent opts)
                            ChildOfMapValueInline vAv
             <> emitTrailingComment (amTrailing e)
...
verbatimEntry opts msrc av
  ...
  , Just sp  <- avSpan av
  , Just src <- msrc
  , Just sl  <- spanSlice src sp = Just sl
  | otherwise = Nothing
...
astripAnnotationsForFlow = astrip
  where
    astrip av = case avBody av of
      ...
      AMap es _         -> YMap (V.map (\e -> (astrip (amKey e),
                                                astrip (amValue e))) es)
...
emitAComments ind cs =
  mconcat
    [ spaces (max ind (commentColumn c))
        <> singleton '#' <> singleton ' '
        <> fromText (commentText c) <> singleton '\n'
    | c <- cs
    ]
