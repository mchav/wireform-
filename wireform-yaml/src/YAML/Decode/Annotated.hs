...
import YAML.Annotated
  ( ABody (..), AMapEntry (..), ADocument (..), AStream (..)
  , AValue (..), Comment (..), MapStyle (..), ScalarStyle (..)
...
decodeAnnotated src = do
  ...
  case V.toList docs of
    ...
    []   -> pure ( ADocument False False
                     (annotatePlain YNull)
                     [] []
                 , src)
    _    -> Left "decodeAnnotated: source contains multiple documents; \
      ...
      | otherwise =
          case lineSliceUtf8 src lineIdx lno of
            ...
            Just lt ->
              ...
              in
                -- Skip directive / doc-marker lines.
                case T.uncons stripped of
                  _ | T.null stripped ->
                        go (lno + 1) pendingComments
                                     (pendingBlanks + 1)
                                     remaining
                  Just ('#', rest) ->
                  ...
                  case remaining of
                    ...
                    (vv : rest) ->
                      let !entryStartByte = V.unsafeIndex starts (lno - 1)
                          (endLine, _) =
                            scanEntryLines src lineIdx (lno + 1)
                          !entryEndByte = V.unsafeIndex starts
                            (min (endLine - 1) (V.length starts - 1))
                          av = annotateContainedValue src lineIdx vv
                                  (Just (SrcSpan
                                           entryStartByte
                                           entryEndByte))
                          av' = av { avTrivia = Trivia
                                       pendingComments
                                       pendingBlanks
                                       Nothing
                                   , avSpan = Just (SrcSpan
...
matchKeyOnLine line key = do
  keyText <- case key of
    ...
    _         -> Nothing
  let plainOk = T.isPrefixOf keyText line
              && hasColonAfter (T.length keyText) line
      sqOk = T.isPrefixOf (squote keyText) line
              && hasColonAfter (T.length keyText + 2) line
      dqOk = T.isPrefixOf (dquote keyText) line
              && hasColonAfter (T.length keyText + 2) line
