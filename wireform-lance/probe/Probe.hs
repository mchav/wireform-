...
import Lance.Manifest qualified as LM
import Lance.Pb.Lance.Table qualified as Pb
              ...
              KM.fromList
                ...
                ,
                  ...
                  , Aeson.Array
                      ( V.fromList
                          ( map
                              (Aeson.Number . fromIntegral . LM.columnMetadataPageLength)
...
parseArgs args0 = case args0 of
  ...
  where
    autoDetect i o = do
      ...
      if isDir
        then pure (Dataset, i, o)
        else
          if takeExtension i == ".lance"
            then pure (File, i, o)
...
datasetJSON
  ...
  -> Aeson.Value
datasetJSON ds mManifest mErr activeFilesJ schemaJ writerVerJ tsMillisJ =
  Aeson.Object $
    KM.fromList
      [ (Key.fromString "mode", Aeson.String "dataset")
      , (Key.fromString "root", textPath (LIO.ldRoot ds))
      ,
        ( Key.fromString "latest_version"
        , case LIO.ldLatestVersion ds of
      ...
      ,
        ( Key.fromString "latest_manifest_footer"
        , case LIO.ldLatestManifestFooter ds of
      ...
      ,
        ( Key.fromString "manifest_decode_error"
        , case mErr of
...
manifestJSON m =
  Aeson.Object $
    KM.fromList
      ...
      ,
        ( Key.fromString "max_fragment_id"
        , case Pb.manifestMaxFragmentId m of
      ...
      ,
        ( Key.fromString "writer_version"
        , case Pb.manifestWriterVersion m of
            Just w -> writerVersionJSON w
            Nothing -> Aeson.Null
        )
      ,
        ( Key.fromString "data_format"
        , case Pb.manifestDataFormat m of
...
writerVersionJSON wv =
  Aeson.Object $
    KM.fromList
      ...
      ,
        ( Key.fromString "prerelease"
        , case Pb.manifestWriterVersionPrerelease wv of
            Just t -> Aeson.String t
            Nothing -> Aeson.Null
        )
      ,
        ( Key.fromString "build_metadata"
        , case Pb.manifestWriterVersionBuildMetadata wv of
            Just t -> Aeson.String t
            Nothing -> Aeson.Null
        )
      ]
