...
import Lance.Format qualified as LF
import Lance.IO (LanceDataset (..), openLanceManifest)
import Lance.Pb.Lance.File qualified as PbFile
import Lance.Pb.Lance.File2 qualified as PbF2
import Lance.Pb.Lance.Table qualified as Pb
import Proto.Decode qualified as ProtoDecode
import Proto.Google.Protobuf.Timestamp qualified as PbTs
...
datasetWriterVersion ds = withActiveManifestM ds $ \m ->
  case Pb.manifestWriterVersion m of
    ...
    Just wv ->
      Just
        ( Pb.manifestWriterVersionLibrary wv
...
datasetTimestampMillis ds = withActiveManifestM ds $ \m ->
  case Pb.manifestTimestamp m of
    ...
    Just ts ->
      ...
      let !s = PbTs.timestampSeconds ts :: Int64
...
readColumnMetadataAt
  :: FilePath
  -> Int
  -- ^ column index (must be < footer.num_columns)
