...
encodingsForTypes
  :: (Int -> Word64)
  -- ^ kind id for column @i@; see 'TypeKind'
  -> Int
  -- ^ total column count
...
encodeStream (Stream kind col len) =
  mconcat
...
encodeColumnEncoding (ColumnEncoding kind dictSz bloomEnc) =
  mconcat
    [ encodeVarintField ColumnEncoding_Kind (columnEncodingKindToInt kind)
    , maybe
        mempty
        (encodeVarintField ColumnEncoding_DictionarySize . fromIntegral)
        dictSz
