...
encodeVarintField (fieldNum, wt) val
  ...
  | otherwise =
      error $
        "ORC.Proto.Schema.encodeVarintField: "
          ++ "field "
          ++ show fieldNum
...
encodeLengthDelim (fieldNum, wt) content
  ...
  | otherwise =
      error $
        "ORC.Proto.Schema.encodeLengthDelim: "
          ++ "field "
          ++ show fieldNum
...
encodeLengthDelimBytes (fieldNum, wt) payload
  ...
  | otherwise =
      error $
        "ORC.Proto.Schema.encodeLengthDelimBytes: "
          ++ "field "
          ++ show fieldNum
          ++ " is not a length-delimited field "
          ++ "(got wire type "
...
encodePackedFixed64Field f@(_, wt) xs
  ...
  | otherwise =
      error $
        "ORC.Proto.Schema.encodePackedFixed64Field: "
          ++ "expected wtLenDelim, got "
          ++ show wt
...
encodeRepeatedFixed64Field (fieldNum, wt) xs
  | wt == wtFixed64 =
      VU.foldl'
        (\b w -> b <> protoTagByte fieldNum wt <> B.word64LE w)
        mempty
        xs
  | otherwise =
      error $
        "ORC.Proto.Schema.encodeRepeatedFixed64Field: "
          ++ "expected wtFixed64, got "
          ++ show wt
