...
fromORCField v pos (FieldInfo mSel ty) = do
  ...
  if miSkip mi
    then case miDefaults mi of
      ...
      Nothing -> do
        let msg =
              "ORC.Derive: missing 'defaults' for skipped field "
                ++ nameBase selName
      ...
      base <-
        [|
          if V.length $(varE v) > $posLit
            then $(parseAtPos v pos mi ty)
            else
              Left
                ( "ORC.Derive: row missing leaf at index "
                    ++ show ($posLit :: Int)
                    ++ " for field "
...
requireSelector Nothing =
  fail
    "ORC.Derive: cannot derive ORC instances for a positional \
...
rejectMsg nm shape =
  "ORC.Derive: cannot derive ORC instances for "
    ++ shape
    ++ " type "
    ++ show nm
...
newtypeNotRecord nm =
  "ORC.Derive: "
    ++ show nm
    ++ " is a newtype; only records carry "
