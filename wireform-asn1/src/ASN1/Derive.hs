  ...
  where
    ...
    sumMatchTo (defaultIdx, c) = do
      ...
      inner <- case fieldNames of
        []   -> [| AV.Null |]
        [n]  -> [| toASN1 $(varE n) |]
        ns   -> [| AV.Sequence (V.fromList
                    $(pure (ListE (map (AppE (VarE 'toASN1) . VarE) ns)))) |]
      bodyE <- [| AV.Tagged AV.ContextSpecific
...
data FieldPlan
  ...
  | -- | Field consumes one slot of the encoded SEQUENCE. Carries the
    ...
    -- messages).
    PlanActive
      !Int          -- ^ active index in the SEQUENCE
      !(Maybe Int)  -- ^ context-specific tag, if any
      !Bool         -- ^ apply 'coerce' to the parsed value
      !Name         -- ^ selector name (for error messages)
...
fromASN1Record c = do
  ...
  lamE [varP v]
    (caseE (varE v)
       [ match (conP 'AV.Sequence [varP vec])
               (normalB
                  [| if V.length $(varE vec)
                          == $(litE (integerL (fromIntegral activeCount)))
                       then $(pure bodyE)
                       else Left ("ASN1.Derive: expected SEQUENCE of "
                                  ++ show ($(litE (integerL (fromIntegral activeCount)))
                                            :: Int)
                                  ++ " elements, got "
        ...
        then do
          let skipExpr = case miDefaults mi of
                Just defNm ->
                  [| Right $(varE defNm) |]
                Nothing    ->
                  [| Left ("ASN1.Derive: missing 'defaults' for skipped field "
  ...
  where
    ...
    sumDispatch tagVar innerVar (defaultIdx, c) = do
      ...
      let i = case miTag mi of
            Just t  -> t
            Nothing -> defaultIdx
          guardE = InfixE (Just (VarE tagVar)) (VarE '(==))
  ...
  [| case $(varE innerVar) of
       AV.Sequence $(varP vec)
         | V.length $(varE vec) == $(litE (integerL (fromIntegral arity)))
             -> $(pure body)
         | otherwise
             -> Left ("ASN1.Derive: " ++ conNameStr
                      ++ " expected " ++ arityStr ++ " contents, got "
                      ++ show (V.length $(varE vec)))
       _ -> Left ("ASN1.Derive: " ++ conNameStr
