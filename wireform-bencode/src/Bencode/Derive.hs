...
fieldParser kvs (FieldInfo mSel _) = do
  ...
  if miSkip mi
    then case miDefaults mi of
      ...
      Nothing    -> [| Left ("Bencode.Derive: missing 'defaults' for skipped field " ++
      ...
      base <-
        if isOptional
          then [| case lookupBencodeField (TE.encodeUtf8 $(pure keyExp)) $(varE kvs) of
                    Nothing -> Right Nothing
                    Just  v -> fmap Just (B.fromBencode v) |]
          else [| case lookupBencodeField (TE.encodeUtf8 $(pure keyExp)) $(varE kvs) of
                    Nothing -> Left ("Bencode.Derive: missing field "
  ...
  [| case $(varE cVar) of
       BV.BList $(varP arr)
         | V.length $(varE arr) == $(litE (integerL (fromIntegral arity)))
             -> $(pure body)
         | otherwise
             -> Left ("Bencode.Derive: " ++ conNameStr
                      ++ " expected " ++ arityStr ++ " contents, got "
                      ++ show (V.length $(varE arr)))
       _ -> Left ("Bencode.Derive: " ++ conNameStr
