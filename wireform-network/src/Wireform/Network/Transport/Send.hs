...
import Control.Exception (IOException, SomeException, toException, try)
import Data.Bits ((.&.))
...
withSendBufTransport
  ...
  -> SendFn
  -> IO ()
  -- ^ shutdownWrite action (e.g. socket SHUT_WR / TLS close_notify)
  -> (SendTransport -> IO a)
  -> IO a
          ...
          loop !cur
            ...
            | otherwise = do
                let !off = fromIntegral cur .&. msk
                    ...
                    !want =
                      min
                        (fromIntegral hd - fromIntegral cur)
                        (sz - off)
