...
import HTML.Class qualified as H
import HTML.Value qualified as HV
...
fromHTMLRecord c = do
  ...
  lamE
    ...
    ( caseE
        (varE v)
        [ match
            (conP 'HV.HTMLElement [wildP, varP attrsVar, varP childrenVar])
            (normalB (pure bodyE))
            []
        , match
            wildP
  ...
  if miSkip mi
    then case miDefaults mi of
      ...
      Nothing ->
        [|
          Left
            ( "HTML.Derive: missing 'defaults' for skipped field "
        ...
        _ ->
          [|
            case lookupHTMLChild $(pure keyExp) $(varE childrenVar) of
              Nothing ->
                Left
                  ( "HTML.Derive: missing child element "
                      ++ T.unpack $(pure keyExp)
                  )
...
fromHTMLEnum cs = do
  ...
  lamE
    ...
    ( caseE
        (varE v)
        [ match
            (conP 'HV.HTMLElement [varP s, wildP, wildP])
            (normalB (pure multi))
            []
        , match
            wildP
...
fromHTMLSum cs = do
  ...
  lamE
    ...
    ( caseE
        (varE v)
        [ match
            (conP 'HV.HTMLElement [varP tagVar, wildP, varP childrenVar])
            (normalB (pure multi))
            []
        , match
            wildP
      ...
      then $(pure body)
        ...
        Left
          ( "HTML.Derive: "
              ++ conNameStr
              ++ " expected "
              ++ show $(litE (integerL (fromIntegral arity)))
              ++ " children, got "
