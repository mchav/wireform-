...
sumCtorToBSON c = do
  ...
  contentsE <- case fieldNames of
    []   -> [| BV.Null |]
    [n]  -> [| B.toBSON $(varE n) |]
    ns   -> [| BV.Array (V.fromList
                $(pure (ListE (map (AppE (VarE 'B.toBSON) . VarE) ns)))) |]
  body <- [| BV.Document (V.fromList
...
fromBSONRecord c = do
  ...
  lamE [varP v]
    (caseE (varE v)
       [ match (conP 'BV.Document [varP kvs])
               (normalB (pure bodyE))
               []
       , match wildP
               (normalB
...
fieldParser kvs (FieldInfo mSel _) = do
  ...
  if miSkip mi
    then case miDefaults mi of
      ...
      Nothing    -> [| Left ("BSON.Derive: missing 'defaults' for skipped field " ++
      ...
      base <-
        if isOptional
          then [| case lookupBSONField $(pure keyExp) $(varE kvs) of
                    Nothing -> Right Nothing
                    Just  v -> fmap Just (B.fromBSON v) |]
          else [| case lookupBSONField $(pure keyExp) $(varE kvs) of
                    Nothing -> Left ("BSON.Derive: missing field "
  ...
  [| case $(varE cVar) of
       BV.Array $(varP arr)
         | V.length $(varE arr) == $(litE (integerL (fromIntegral arity)))
             -> $(pure body)
         | otherwise
             -> Left ("BSON.Derive: " ++ conNameStr
                      ++ " expected " ++ show arity ++ " contents, got "
                      ++ show (V.length $(varE arr)))
       _ -> Left ("BSON.Derive: " ++ conNameStr
