...
import Data.ByteString.Lazy.Internal qualified as BSLI
...
import Network.HTTP.ContentCoding qualified as Hermes
    ...
    then sendRaw inner req
      ...
      let token = ehToken handler
          ...
          hdrs2 =
            H.insertHeader
              H.hContentLength
              (BS8.pack (show (BS.length encoded)))
              hdrs1
      sendRaw
        inner
        req
