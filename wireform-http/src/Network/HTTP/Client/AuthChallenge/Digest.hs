...
buildResponse
  :: DigestState
  -> AuthChallenge
  -> ByteString  -- ^ algorithm token, e.g. @"SHA-256"@
  -> ByteString  -- ^ user
  -> ByteString  -- ^ password
  -> ByteString  -- ^ method
  -> ByteString  -- ^ digest-uri
  -> IO ByteString
buildResponse state ch algoBs user password method uri = do
  let realm    = paramOr "" "realm"    ch
      ...
      userhash = (CI.mk <$> lookup (CI.mk "userhash") (acParams ch))
                   == Just (CI.mk "true")
