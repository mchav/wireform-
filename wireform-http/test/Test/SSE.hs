...
import Network.HTTP.Types.Body qualified as TB
...
tests :: Spec
tests =
  describe "Network.HTTP.Client.SSE" $
    sequence_
      [ parserBasicTests
  ...
  describe "parser/BOM" $
    sequence_
      ...
      , it "BOM split across chunks" $ do
          ...
          events (fs1 <> fs2 <> fs3)
            `shouldBe` [defaultSseEvent {sseData = "y"}]
...
chunkBoundaryTests =
  describe "parser/chunk boundaries" $
    sequence_
      ...
      , it "non-2xx status raises SseUnexpectedStatus" $ do
          let base =
                chunkTransport
                  ...
                  []
          result <-
            try (withSSE base dummyReq $ \_ -> pure ())
...
mediaTypeTests :: Spec
mediaTypeTests =
  describe "MediaType integration" $
    sequence_
      ...
      , it "as @EventStream decodes a fixture body" $ do
          ...
          evs
            `shouldBe` [ defaultSseEvent {sseEventType = Just "tick", sseData = "1"}
  ...
  describe "SseChannel" $
    sequence_
      ...
      , it "producer in another thread, bounded backpressure" $ do
          ...
          let evs =
                map
                  (\i -> defaultSseEvent {sseData = BS8.pack ("e" <> show i)})
                  [0 .. 19 :: Int]
