...
import Network.HTTP.Headers.Parsing.Util qualified as FP
...
tests :: Spec
tests =
  describe "Network.HTTP.URL.Decode" $
    sequence_
      [ pureDecoderTests
      , scannerTests
      , errorTests
...
pureDecoderTests =
  describe "urlDecode / urlDecodeForm" $
    sequence_
...
flatparseTests :: Spec
flatparseTests =
  describe "flatparse combinators" $
    sequence_
