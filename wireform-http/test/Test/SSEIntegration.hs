...
import Control.Exception (SomeException, bracket, finally, fromException, try)
...
tests :: Spec
tests =
  describe "SSE end-to-end (HTTP/1.1)" $
    sequence_
      [ basicRoundTrip
      , heartbeatsAndEvents
...
backToBackRequests = it "3 sequential SSE requests against the same server" $ do
  let runs =
        [ map
            (\i -> defaultSseEvent {sseData = BS8.pack ("a" <> show i)})
            [0 .. 99 :: Int]
        , map
            (\i -> defaultSseEvent {sseData = BS8.pack ("b" <> show i)})
            [0 .. 199 :: Int]
        , map
            (\i -> defaultSseEvent {sseData = BS8.pack ("c" <> show i)})
            [0 .. 299 :: Int]
        ]
