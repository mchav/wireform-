...
sumCtorToIon c = do
  ...
  contentsE <- case fieldNames of
    []   -> [| IV.Null |]
    [n]  -> [| I.toIon $(varE n) |]
    ns   -> [| IV.List (V.fromList
                $(pure (ListE (map (AppE (VarE 'I.toIon) . VarE) ns)))) |]
  body <- [| IV.Struct (V.fromList
...
fieldParser kvs (FieldInfo mSel _) = do
  ...
  if miSkip mi
    then case miDefaults mi of
      ...
      Nothing    -> [| Left ("Ion.Derive: missing 'defaults' for skipped field " ++
      ...
      base <-
        if isOptional
          then [| case lookupIonField $(pure keyExp) $(varE kvs) of
                    Nothing -> Right Nothing
                    Just  v -> fmap Just (I.fromIon v) |]
          else [| case lookupIonField $(pure keyExp) $(varE kvs) of
                    Nothing -> Left ("Ion.Derive: missing field "
  ...
  [| case $(varE cVar) of
       IV.List $(varP arr)
         | V.length $(varE arr) == $(litE (integerL (fromIntegral arity)))
             -> $(pure body)
         | otherwise
             -> Left ("Ion.Derive: " ++ conNameStr
                      ++ " expected " ++ show arity ++ " contents, got "
                      ++ show (V.length $(varE arr)))
       _ -> Left ("Ion.Derive: " ++ conNameStr
