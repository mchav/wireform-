...
import Parquet.Page qualified as PP
...
data NestedRow
  = -- | a primitive value (matched against an NSPrimitive node)
    NRLeaf !LeafValue
  | -- | a Nothing at an NSOptional wrapper
    NRNull
  | -- | a list (possibly empty)
    NRList !(V.Vector NestedRow)
  | -- | struct fields, indexed in NSStruct order
  ...
  where
    ...
    go path !d !r = \case
      ...
      NSList inner ->
        ...
        in -- 'list' is a repeated group, so descending into it
           -- bumps both d and r.
           go path' (d + 1) (r + 1) inner
      NSMap k v ->
...
descendThrough
  ...
  -- ^ current def
  -> Int
  -- ^ rep level for the next leaf event
  -> Int
  -- ^ absolute rep depth
...
descendThrough schema !cursor !accs row !d !r !absRep = case (schema, row) of
  ...
  (NSList inner, NRList children)
    | V.null children ->
        Right
          ( emitNullsAcrossSubtree inner cursor accs d r
    ...
    | V.null pairs ->
        Right
          ( emitNullsAcrossSubtree
              (NSStruct (V.fromList [("key", k), ("value", v)]))
              cursor
              accs
              d
              r
              ...
              !pathedLeaves =
                concat
                  ( zipWith
                      ( \name leaves ->
                          map
                            (\l -> l {nlPath = V.cons name (V.tail (nlPath l))})
                            (V.toList leaves)
                      )
                      columnNames
                      leavesByCol
