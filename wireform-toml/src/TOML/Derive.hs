  ...
  if miSkip mi
    then case miDefaults mi of
      ...
      Nothing ->
        [|
          Left
            ( "TOML.Derive: missing 'defaults' for skipped field "
      ...
      base <-
        if isOptional
          then
            [|
              case lookupTOMLField $(pure keyExp) $(varE kvs) of
