...
| `wireform-fury`       | `Fury.Class`, `Fury.Encode`, `Fury.Decode`, `Fury.Derive`, `Fury.Encoding`, `Fury.MetaString`, `Fury.TypeId`, `Fury.Value`                                                                                                                                                                                                                                                                                                                                                                                                                       | Apache Fory (formerly Fury) xlang serialization. Wire-compatible with `pyfory` 0.17 for top-level `null`, `bool`, `int*`/`varint*`, `uint*`/`varuint*`, `float32`/`float64`, `string` (with the LATIN-1 / UTF-8 encoding selection pyfory uses), `binary`, `LIST` (homogeneous and heterogeneous, with-nulls and no-nulls), and `MAP` (chunked key-type/value-type format). Verified by the `wireform-fury-interop` test suite which round-trips a representative value set in both directions through pyfory; see `wireform-fury/test-interop/`. The package additionally implements `NAMED_STRUCT` and `NAMED_COMPATIBLE_STRUCT` framing with shared `TypeDef` sidecar, reference tracking via `RefVal` (also exposed as `Fury.Class.Shared`), meta-string deduplication, and the canonical 1-D `*_ARRAY` type IDs in a self-consistent format that is round-trippable inside this package but not yet wire-compatible with pyfory (those features use formats — bit-packed `TypeDef` field-info, in-collection ref tracking, NumPy-typed array routing — that are deferred). See `Fury.Encode`'s haddock for the exact wire shape. |
