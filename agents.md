...
| `wireform-delta`      | `Delta.Log` (typed actions: `add` / `remove` / `metaData` / `protocol` / `commitInfo` / `txn` / `cdc`; `parseLogLine` / `parseLogFile`; `TableSnapshot` + `applyAction` / `snapshotFromActions`; `parseDeltaSchema` for the JSON `schemaString`; `AddStats` decoder; `LastCheckpoint` for `_delta_log/_last_checkpoint`) | Delta Lake transaction log reader. Interop against `deltalake` (delta-rs) lives in `wireform-delta/probe/Probe.hs` + `wireform-delta/scripts/delta_interop.py`. Out of scope so far: checkpoint Parquet decode, deletion vectors, column mapping, time travel by timestamp, V2 checkpoint format. |
| `wireform-hudi`       | `Hudi.Timeline` (`parseInstantFileName` for both implicit-completed and explicit-state forms; `sortInstants` / `completedInstants`; `HoodieCommitMetadata` + `HoodieWriteStat` JSON decoders; `FileSlice` / `TableState` + `applyCommit` / `tableStateFromCommits` for the partition→fileId→slice fold)             | Apache Hudi timeline reader (Copy-on-Write). Interop against `hudi-rs`'s Python bindings lives in `wireform-hudi/probe/Probe.hs` + `wireform-hudi/scripts/hudi_interop.py`. Out of scope so far: Avro instant payloads (Hudi 1.x+), MoR log-block decoding, record-level merge keys, the metadata table. |
| `wireform-lance`      | `Lance.Format` (envelope + 40-byte fixed footer per the spec at <https://lance.org/format/file/>: `lanceMagic`, `LanceFile`, `LanceFooter`, `parseFooter`, `parseColumnOffsetTable`, `parseGlobalBufferOffsetTable`, `extractColumnMetadataBytes`)                                                                  | Apache Lance file reader. Interop against `pylance` lives in `wireform-lance/probe/Probe.hs` + `wireform-lance/scripts/lance_interop.py`. The protobuf `ColumnMetadata` decoder lives downstream (so the package doesn't depend on `wireform-proto`); this module exposes the byte ranges that decoder consumes. |
