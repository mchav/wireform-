...
| `wireform-delta`      | `Delta.Log` (typed actions: `add` / `remove` / `metaData` / `protocol` / `commitInfo` / `txn` / `cdc`; `parseLogLine` / `parseLogFile`; `TableSnapshot` + `applyAction` / `snapshotFromActions`; `parseDeltaSchema` for the JSON `schemaString`; `AddStats` decoder; `LastCheckpoint` for `_delta_log/_last_checkpoint`); `Delta.IO` (`openDeltaTable`, `findCommits`, `findLastCheckpoint`, `listLogEntries` — IO entry points that walk a table directory and replay every commit into a `TableSnapshot`). | Delta Lake transaction log reader. Interop against `deltalake` (delta-rs) lives in `wireform-delta/probe/Probe.hs` + `wireform-delta/scripts/delta_interop.py` and exercises both unpartitioned/partitioned tables and a checkpointed table (where `dtLastCheckpoint` + `dtCheckpointAvailable` are populated). Out of scope so far: decoding the body of `*.checkpoint.parquet` (the JSON-walk replay still works on checkpointed tables but is O(N) commits rather than O(1)), deletion vectors, column mapping, time travel by timestamp, V2 checkpoint format. |
