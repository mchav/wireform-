...
| `wireform-hudi`       | `Hudi.Timeline` (`parseInstantFileName` for both implicit-completed and explicit-state forms; `sortInstants` / `completedInstants`; `HoodieCommitMetadata` + `HoodieWriteStat` JSON decoders; `FileSlice` / `TableState` + `applyCommit` / `tableStateFromCommits` for the partition→fileId→slice fold); `Hudi.IO` (`openHudiTable`, `scanTimeline`, `readHoodieProperties`, `tableSchemaFromCommits`).                | Apache Hudi timeline reader (Copy-on-Write). Interop against `hudi-rs`'s Python bindings lives in `wireform-hudi/probe/Probe.hs` + `wireform-hudi/scripts/hudi_interop.py`. Out of scope so far: Avro instant payloads (Hudi 1.x+), MoR log-block decoding, record-level merge keys, the metadata table. |
| `wireform-lance`      | `Lance.Format` (data-file envelope + 40-byte data-file footer + 16-byte manifest footer per the spec at <https://lance.org/format/file/>: `lanceMagic`, `LanceFile`, `LanceFooter`, `LanceManifestFooter`, `parseFooter`, `parseManifestFooter`, `parseColumnOffsetTable`, `parseGlobalBufferOffsetTable`, `extractColumnMetadataBytes`); `Lance.IO` (`openLanceFile`, `openLanceManifest`, `openLanceDataset`, `findManifestVersions`, `decodeManifestFileName`/`encodeManifestFileName` for the @2^64 − 1 − v@ manifest filename convention).        | Apache Lance file + dataset reader. Interop against `pylance` lives in `wireform-lance/probe/Probe.hs` + `wireform-lance/scripts/lance_interop.py` and covers both `--file` (single `.lance` data file) and `--dataset` (the `_versions/` manifest enumeration + `data/*.lance` file listing). The protobuf `ColumnMetadata` and `Manifest` decoders live downstream (so the package doesn't depend on `wireform-proto`); this module exposes the byte ranges those decoders consume. |
