...
| `wireform-cel` | `CEL`, `CEL.{Value,Syntax,Parser,Eval,Stdlib,Environment,Error}` | A conformant [Common Expression Language](https://github.com/google/cel-spec/blob/master/doc/langdef.md) parser + evaluator over a dynamic `Value` model: full grammar/lexis, number-line numeric semantics (`1 == 1u == 1.0`, NaN unordered), error-absorbing `&&`/`||`, the comprehension macros (`has`/`all`/`exists`/`exists_one`/`map`/`filter`), and the standard library of operators, conversions, string/regex functions, and `Timestamp`/`Duration` support. Depends only on Hackage (no internal `wireform-*` deps), so it builds standalone. Not yet: protobuf message values, named timezones, the optional type-checker. |
