...
| `wireform-fury`       | `Fury.Class`, `Fury.Encode`, `Fury.Decode`, `Fury.Derive`, `Fury.Encoding`, `Fury.MetaString`, `Fury.TypeId`, `Fury.Value`                                                                                                                                                                                                                                                                                                                                                                                                                       | Apache Fory (formerly Fury) xlang serialization. Covers the full primitive value layer (varuint32/64, zigzag varint, tagged int64, varuint36_small, IEEE float/double, UTF-8 string headers), `NAMED_STRUCT` and `NAMED_COMPATIBLE_STRUCT` framing with shared `TypeDef` sidecar, reference tracking via `RefVal` (and the `Fury.Class.Shared` newtype), meta-string deduplication, and the canonical 1-D `*_ARRAY` type IDs. Intentional simplifications: meta strings are UTF-8 only and skip the spec's 64-bit content hash for >16-byte strings; `TypeDef` field-info uses a flattened `meta-string + type-id` layout rather than the spec's bit-packed field header. See `Fury.Encode`'s haddock for the exact wire shape. |
