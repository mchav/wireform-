...
| `wireform-fury`       | `Fury.Class`, `Fury.Encode`, `Fury.Decode`, `Fury.Derive`, `Fury.Encoding`, `Fury.MetaString`, `Fury.TypeId`, `Fury.Value`                                                                                                                                                                                                                                                                                                                                                                                                                       | Apache Fory (formerly Fury) xlang serialization. Initial scope: primitive value layer (varuint32/64, zigzag varint, tagged int64, varuint36_small, IEEE float/double, UTF-8 string headers) plus `NAMED_STRUCT` framing. Reference tracking, meta-string deduplication, and the streaming `TypeDef` cache are deliberately not implemented yet — see `Fury.Encode`'s haddock for the exact subset. |
