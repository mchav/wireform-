...
| `wireform-delta`      | `Delta.Log` (typed actions: `add` / `remove` / `metaData` / `protocol` / `commitInfo` / `txn` / `cdc`; `parseLogLine` / `parseLogFile`; `TableSnapshot` + `applyAction` / `snapshotFromActions`; `parseDeltaSchema` for the JSON `schemaString`; `AddStats` decoder; `LastCheckpoint` for `_delta_log/_last_checkpoint`); `Delta.Checkpoint` (path-aware decoder for `*.checkpoint.parquet` rows — `add` / `remove` / `metaData` / `protocol` actions reconstructed straight from the columnar snapshot, including the `map<string, string>` / `list<string>` leaves: `partitionValues` / `tags` / `partitionColumns` / `configuration` / `readerFeatures` / `writerFeatures`); `Delta.IO` (`openDeltaTable`, `findCommits`, `findLastCheckpoint`, `listLogEntries` — opens a table by checkpoint-short-circuit when a checkpoint Parquet is present, otherwise full JSON replay). | Delta Lake transaction log reader. Interop against `deltalake` (delta-rs) lives in `wireform-delta/probe/Probe.hs` + `wireform-delta/scripts/delta_interop.py` and covers (a) unpartitioned tables, (b) partitioned tables, (c) a checkpointed table at v11 with APPEND v12 + OVERWRITE v13 (exercising both the checkpoint short-circuit and the JSON-tail replay), and (d) a partitioned + checkpointed table that cross-checks the typed `partitionValues` map and `partitionColumns` list out of the checkpoint Parquet against `DeltaTable.metadata().partition_columns`. Out of scope so far: deletion vectors, column mapping, time travel by timestamp, V2 multi-part checkpoint format. |
| `wireform-hudi`       | `Hudi.Timeline` (`parseInstantFileName` for both implicit-completed and explicit-state forms; `sortInstants` / `completedInstants`; `HoodieCommitMetadata` + `HoodieWriteStat` JSON decoders; `FileSlice` / `TableState` + `applyCommit` / `tableStateFromCommits` for the partition→fileId→slice fold); `Hudi.Avro` (Avro container decoder for the Hudi 1.x+ instant payload format, schema vendored under `avro/HoodieCommitMetadata.avsc` and embedded via `Data.FileEmbed`); `Hudi.IO` (`openHudiTable`, `scanTimeline`, `readHoodieProperties`, `tableSchemaFromCommits`).                | Apache Hudi timeline reader (Copy-on-Write). Interop against `hudi-rs`'s Python bindings lives in `wireform-hudi/probe/Probe.hs` + `wireform-hudi/scripts/hudi_interop.py` and covers both Hudi 0.x JSON instants and Hudi 1.x+ Avro instants. Out of scope so far: MoR log-block decoding, record-level merge keys, the metadata table. |
| `wireform-lance`      | `Lance.Format` (data-file envelope + 40-byte data-file footer + 16-byte manifest footer per the spec at <https://lance.org/format/file/>: `lanceMagic`, `LanceFile`, `LanceFooter`, `LanceManifestFooter`, `parseFooter`, `parseManifestFooter`, `parseColumnOffsetTable`, `parseGlobalBufferOffsetTable`, `extractColumnMetadataBytes`); `Lance.IO` (`openLanceFile`, `openLanceManifest`, `openLanceDataset`, `findManifestVersions`, `decodeManifestFileName`/`encodeManifestFileName` for the @2^64 − 1 − v@ manifest filename convention); `Lance.Manifest` (typed protobuf decoder for the `lance.table.Manifest` body — version, fragments, writer version, data format, transaction file, etc.); `Lance.Pb.Lance.{File,Table}` (auto-generated by `cabal run wireform-lance:gen-lance-pb` from `proto/lance/{file,table}.proto`).        | Apache Lance file + dataset reader. Interop against `pylance` lives in `wireform-lance/probe/Probe.hs` + `wireform-lance/scripts/lance_interop.py` and covers both `--file` (single `.lance` data file) and `--dataset` (the `_versions/` manifest enumeration + `data/*.lance` file listing + the typed protobuf `Manifest` body cross-checked against `lance.dataset(...).get_fragments()`). The protobuf `ColumnMetadata` decoder for individual data files still lives downstream; this module exposes the byte ranges that decoder would consume. |
