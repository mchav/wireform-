...
data LZ4Error
  = -- | liblz4 returned a negative size; the value is the magnitude.
    LZ4DecodeFailed !Int
  | -- | Caller-supplied @maxOutput@ was negative.
    LZ4OutputOverrun
  | -- | liblz4 returned 0 from compress (bound too small).
    LZ4CompressFailed
  deriving (Show, Eq)
...
prettyLZ4Error = \case
  LZ4DecodeFailed n ->
    "LZ4: decompression failed at byte "
      ++ show n
