...
sumCtorToEDN c = do
  ...
  contentsE <- case fieldNames of
    []   -> [| EV.Nil |]
    [n]  -> [| E.toEDN $(varE n) |]
    ns   -> [| EV.Vector (V.fromList
                $(pure (ListE (map (AppE (VarE 'E.toEDN) . VarE) ns)))) |]
  body <- [| EV.Map (V.fromList
...
fieldParser kvs (FieldInfo mSel _) = do
  ...
  if miSkip mi
    then case miDefaults mi of
      ...
      Nothing    -> [| Left ("EDN.Derive: missing 'defaults' for skipped field " ++
      ...
      base <-
        if isOptional
          then [| case lookupEDNField (EV.Keyword Nothing $(pure keyExp))
                                       $(varE kvs) of
                    Nothing -> Right Nothing
                    Just  v -> fmap Just (E.fromEDN v) |]
          else [| case lookupEDNField (EV.Keyword Nothing $(pure keyExp))
                                       $(varE kvs) of
                    Nothing -> Left ("EDN.Derive: missing field :"
...
fromEDNEnum cs = do
  ...
  lamE [varP v]
    (caseE (varE v)
       [ match (conP 'EV.Keyword [wildP, varP s])
               (normalB (pure multi))
               []
       , match wildP
  ...
  [| case $(varE cVar) of
       EV.Vector $(varP arr)
         | V.length $(varE arr) == $(litE (integerL (fromIntegral arity)))
             -> $(pure body)
         | otherwise
             -> Left ("EDN.Derive: " ++ conNameStr
                      ++ " expected " ++ show arity ++ " contents, got "
                      ++ show (V.length $(varE arr)))
       _ -> Left ("EDN.Derive: " ++ conNameStr
