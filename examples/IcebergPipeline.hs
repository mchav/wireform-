...
import Iceberg.Catalog.REST qualified as REST
import Iceberg.Manifest qualified as IM
import Iceberg.Parquet qualified as IP
...
main = do
  ...
  let manifestEntry =
      ...
      stats =
        (IU.statsFromManifestEntry manifestEntry)
  ...
  let table1Compressed =
        table1
          { I.tmProperties =
              Map.insert
                "write.metadata.compression-codec"
                "gzip"
                (I.tmProperties table1)
          }
      tableJson = IW.encodeTableMetadata table1Compressed
      tableJsonZ = IW.encodeTableMetadataCompressed table1Compressed
  putStrLn $
    "TableMetadata JSON:        "
      ++ show (BS.length tableJson)
      ++ " bytes (gzip-codec recorded)"
  putStrLn $
    "TableMetadata gzip output: "
      ++ show (BS.length tableJsonZ)
      ++ " bytes"
