    ...
    go (defaultId, FieldInfo mSel _) = do
      ...
      if miSkip mi
        then pure []
          ...
          pair <- [| ( $(litE (integerL (fromIntegral fid))) :: Word16
                     , $btE
                     , $encoded
                     ) |]
          pure [pair]
...
fromBondRecord c = do
  ...
  lamE [varP v]
    (caseE (varE v)
       [ match (conP 'BV.Struct [wildP, varP kvs])
               (normalB (pure bodyE))
               []
       , match wildP
               (normalB
...
fieldParser kvs (defaultId, FieldInfo mSel _) = do
  ...
  if miSkip mi
    then case miDefaults mi of
      ...
      Nothing    -> [| Left ("Bond.Derive: missing 'defaults' for skipped field " ++
      ...
      base <-
        if isOptional
          then [| case lookupBondField $(litE (integerL (fromIntegral fid))) $(varE kvs) of
                    Nothing -> Right Nothing
                    Just  v -> fmap Just (fromBond v) |]
          else [| case lookupBondField $(litE (integerL (fromIntegral fid))) $(varE kvs) of
                    Nothing -> Left ("Bond.Derive: missing field id "
...
fromBondEnum cs = do
  ...
  lamE [varP v]
    (caseE (varE v)
       [ match (conP 'BV.Int32 [varP i])
               (normalB (pure multi))
               []
       , match (conP 'BV.Enum  [varP i])
               (normalB (pure multi))
               []
       , match wildP
...
sumNAry payVar conName arity = do
  ...
  [| do
       ...
       if length $(varE arr) /= $(litE (integerL (fromIntegral arity)))
         then Left ("Bond.Derive: " ++ conNameStr ++ " expected "
