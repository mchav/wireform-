...
tests :: Spec
tests = describe "Aeson deriver" $ sequence_
  [ recordTests
  , newtypeTests
  , enumTests
  , sumTests
...
enumTests = describe "enum" $ sequence_
  [ it "encode literal-renamed constructor"
      (A.toJSON Red `shouldBe` A.String "red")
  , it "encode style-renamed constructor (DarkBlue -> dark-blue)"
