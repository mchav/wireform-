  ...
  subparser
    ( command
        ...
        <> command
          "print"
          ( info
              (ProtoPrint <$> protoPrintOptsParser)
              (progDesc "Parse and exact-print a .proto file")
          )
        <> command
          "summary"
          ( info
              (ProtoSummary <$> protoPrintOptsParser)
...
collectTransitiveImports dirs rp =
  concatMap
    ( \(_, imp) ->
        (stripIncludeDirs dirs (rpPath imp), imp)
