...
import Proto.Temporalio.Api.Namespace.V1.Message qualified as TE_Namespace_V1_Message
...
defaultUnsuccessfulOperationError =
...
instance MessageSize Request where
  messageSize msg =
    (Map.foldlWithKey' (\acc k v -> let entrySz = fieldTextSize 1 k + fieldTextSize 2 v in acc + tagSize 1 + varintSize (fromIntegral entrySz) + entrySz) 0 msg.requestHeader)
      ...
      + ( case msg.requestVariant of
            Nothing -> 0
            Just (Request'Variant'StartOperation v) -> fieldMessageSize 3 (messageSize v)
...
defaultStartOperationResponse'Sync =
...
defaultStartOperationResponse'Async =
...
instance MessageSize StartOperationResponse where
  messageSize msg =
    ( case msg.startOperationResponseVariant of
        Nothing -> 0
        Just (StartOperationResponse'Variant'SyncSuccess v) -> fieldMessageSize 1 (messageSize v)
...
defaultCancelOperationResponse =
...
instance MessageSize Response where
  messageSize msg =
    ( case msg.responseVariant of
        Nothing -> 0
        Just (Response'Variant'StartOperation v) -> fieldMessageSize 1 (messageSize v)
...
defaultEndpointTarget'External =
...
instance MessageSize EndpointTarget where
  messageSize msg =
    ( case msg.endpointTargetVariant of
        Nothing -> 0
        Just (EndpointTarget'Variant'Worker v) -> fieldMessageSize 1 (messageSize v)
...
registerModuleTypes =
  Proto.Registry.registerType (Proxy :: Proxy Failure)
    ...
    . Proto.Registry.registerType (Proxy :: Proxy EndpointTarget'External)
    . id
