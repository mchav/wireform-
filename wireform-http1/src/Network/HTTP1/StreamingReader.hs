...
import Network.HTTP1.Parser qualified as Classic
...
readHeaderBlock
  :: ReceiveTransport
  -> Int
  -- ^ hard cap; @ReadMessageTooLong@ if exceeded.
...
findCRLFCRLF
  ...
  -- ^ start position
  -> Int
  -- ^ scan-from offset (resume after a refill)
  -> Int
  -- ^ bytes available from start
  -> IO (Maybe Int)
...
readUpTo
  ...
  -- ^ start position
  -> Int
  -- ^ maximum bytes to return (clamped to ring size)
  -> IO (Either ReadError (ByteString, Word64))
