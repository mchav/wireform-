...
parseOneHeader raw off0 =
  ...
  in if nameEnd == off0
       then Left ParseBadHeaderName
         ...
         if nameEnd >= len || BSU.unsafeIndex raw nameEnd /= 0x3a -- ':'
           then Left ParseBadHeaderName
           else
          ...
          in if b < 0x30 || b > 0x39
               then Nothing
                 ...
                 let !d = fromIntegral (b - 0x30) :: Word64
                     !acc' = acc * 10 + d
                 in -- overflow guard: max Word64 = 1844...; checking
                    -- acc * 10 + 9 < acc (i.e. acc > maxBound `div` 10)
                    -- is cheap and conservative.
                    if acc > 1844674407370955160 || (acc == 1844674407370955160 && d > 5)
                      then Nothing
                      else go (i + 1) acc'
