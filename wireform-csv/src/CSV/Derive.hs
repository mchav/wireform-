...
toCSVNewtype c = case conInfoFields c of
  ...
  [FieldInfo Nothing _] -> do
    x <- newName "x"
    lamE [conP (conInfoName c) [varP x]]
  ...
  if miSkip mi
    then case miDefaults mi of
      Just defNm -> do
        ...
        pure (e, 0)
      Nothing ->
        let msg = "CSV.Derive: missing 'defaults' for skipped field "
                  ++ nameBase selName
        in (\e -> (e, 0)) <$> [| Left $(litE (stringL msg)) |]
    else do
      let posLit = litE (integerL (fromIntegral pos))
          base = [| if V.length $(varE cells) > $posLit
                      then C.fromCSVField ($(varE cells) V.! $posLit)
                      else Left ("CSV.Derive: row missing cell at index "
                                 ++ show ($posLit :: Int)) |]
