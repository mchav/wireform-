...
import qualified Hedgehog.Internal.Gen as IG
...
genValue depth = Gen.frequency $
  ...
  ++
  [ (2, VV.ListVal . V.fromList <$>
         Gen.list (Range.linear 0 4) (genValue (depth - 1)))
  | depth > 0
  ]
  ++
  [ (1, VV.MapVal . V.fromList <$>
...
main = do
  driver <- findDriver
  (Just hin, Just hout, _, _ph) <- createProcess
    (proc "python3" [driver])
            ...
            case mTree of
              ...
              Just t  -> do
                let v = Tree.treeValue t
                    bytes = E.encode v
                hPutStrLn stderr $ "[case " ++ show i ++ "] "
                                    ++ show v ++ "  ("
                                    ++ show (BS.length bytes) ++ "B)"
                hFlush stderr
...
findDriver = do
  ...
  if exists1 then pure "wireform-fory/test-interop/driver.py"
