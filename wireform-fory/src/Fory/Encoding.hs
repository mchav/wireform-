...
runBuilder (Builder b) =
  BSL.toStrict (BBE.toLazyByteStringWith strat BSL.empty b)
  where
    -- Default first-buffer 4 KiB matches GHC's default and is
    -- enough for almost any fory message produced via this layer
    -- without an extra chunk allocation.
    strat = BBE.untrimmedStrategy 4096 BBE.defaultChunkSize
...
varuint32 = Builder . goVaru32
  where
    goVaru32 !v
      ...
      | otherwise = BB.word8 (fromIntegral (v .&. 0x7F) .|. 0x80)
                 <> goVaru32 (v `shiftR` 7)
{-# INLINE varuint32 #-}
