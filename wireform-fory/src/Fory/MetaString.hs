...
import Fory.MetaString.Hash qualified as Hash
...
readFreshMetaStringPayload
  :: SpecialChars
  -> Int
  -- ^ Byte length, as returned by 'readMetaStringHeader'.
  -> ByteString
  -> Int
...
readFreshMetaStringPayload sc len bs off
  ...
  | len <= smallStringThreshold = do
      ...
      enc <- case encodingFromId encByte of
        ...
        Nothing ->
          Left $
            "Fory.MetaString: unknown encoding id "
              ++ show encByte
  ...
  | otherwise = do
      ...
      enc <- case encodingFromId encByte of
        ...
        Nothing ->
          Left $
            "Fory.MetaString: unknown encoding id from hashcode "
              ++ show encByte
      (raw, off2) <- readBytes len bs off1
      Right (decodeMetaString sc enc raw, off2)
