...
import Fory.MetaString qualified as MS
import Fory.MetaString.Encoder qualified as MSE
  ...
  where
    go = do
      ...
      if hdr .&. 0x02 == 0
        then
          failD
            ( "Fory.Decode.decode: missing xlang flag in header byte "
                ++ show hdr
            )
                    ...
                    then V.replicateM count $ do
                      ...
                      if f == slotNull
                        then pure VV.NoneVal
                          ...
                          if f == slotNotNullValue
                            then elemReader
                            else
                              failD
                                ( "Fory.Decode: unexpected element flag "
                                    ++ show f
                                )
      ...
      case (keyNull, valNull) of
        ...
        (True, False) -> do
          ...
          if flag /= slotNotNullValue
            then
              failD
                ( "Fury.Decode: expected NOT_NULL_VALUE for partial-"
                    ++ "null map value, got "
                    ++ show flag
                )
            else do
                ...
                then pure VV.NoneVal
                  ...
                  if flag == slotNotNullValue
                    then decodePayloadFor (ST.fsTypeId spec)
                      ...
                      failD
                        ( "Fory.Decode: bad nullable basic field flag "
                            ++ show flag
                        )
            ...
            then pure VV.NoneVal
              ...
              if flag == slotNotNullValue
                then decodePayloadFor (ST.fsTypeId spec)
                  ...
                  failD
                    ( "Fory.Decode: bad non-basic field flag "
                        ++ show flag
                    )
...
decodeTypeDefBytes = do
  ...
  if consumed /= bodyLen
    then
      failD $
        "Fory.Decode: TypeDef body size mismatch (header said "
          ++ show bodyLen
          ++ ", consumed "
          ++ show consumed
          ++ ")"
