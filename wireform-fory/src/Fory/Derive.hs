...
fromForyRecord c = do
  ...
  lamE
    ...
    ( caseE
        (varE v)
        [ match
            (conP 'VV.StructVal [wildP, wildP, varP fields])
            (normalB (pure bodyE))
            []
        , match
            wildP
  ...
  if miSkip mi
    then case miDefaults mi of
      ...
      Nothing ->
        [|
          Left
            ( "Fory.Derive: missing 'defaults' for skipped field "
      ...
      base <-
        if isOptional
          then
            [|
              case lookupForyField $(pure keyExp) $(varE fields) of
