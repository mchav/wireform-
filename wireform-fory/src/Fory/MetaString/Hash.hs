...
import Data.Bits (xor, shiftL, shiftR, rotateL, (.&.), (.|.))
...
-- encoding tag.
hashSmallMetaString
  :: Word64  -- ^ v1 (first 8 bytes, little-endian, zero-padded)
  -> Word64  -- ^ v2 (next 8 bytes, little-endian, zero-padded)
  -> Int     -- ^ length in bytes (0..16)
  -> Word64  -- ^ encoding tag (0..4)
  -> Word64
