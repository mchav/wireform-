...
import Data.Char (chr, isDigit, isLower, isUpper, ord, toLower, toUpper)
...
charValue5 c
  ...
  | otherwise =
      error $
        "Fory.MetaString.Encoder.charValue5: unsupported char "
          ++ show c
...
charValue6 (SpecialChars c1 c2) c
  ...
  | otherwise =
      error $
        "Fory.MetaString.Encoder.charValue6: unsupported char "
          ++ show c
  ...
  | otherwise = case enc of
      UTF8 ->
        case TE.decodeUtf8' bs of
          ...
          Left e ->
            error
              ( "Fory.MetaString.Encoder.decodeMetaString UTF8: "
                  ++ show e
              )
