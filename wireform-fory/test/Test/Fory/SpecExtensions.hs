...
metaShareTests :: Spec
metaShareTests =
  describe "meta-string deduplication" $
    sequence_
      [ it "two structs of the same type share namespace + type-name" $ do
          let mkS =
                VV.StructVal
                  "long.namespace.path.example"
                  "MyStruct"
              ...
              overheadBound = 16
          ( if (twoBytes - oneBytes <= overheadBound)
              then pure ()
              else
                expectationFailure
...
compatibleStructTests =
  describe "NAMED_COMPATIBLE_STRUCT" $
    sequence_
      [ it "single CompatibleStructVal round-trips" $ do
          let v =
                VV.CompatibleStructVal
                  "x.y"
                  "T"
                  ( V.fromList
                      [ ("a", VV.Int32Val 1)
  ...
  describe "primitive 1-D arrays" $
    sequence_
      ...
      , it "Int32Array is denser than equivalent ListVal of Int32Val" $ do
          ...
          ( if (arrBytes < listBytes)
              then pure ()
              else
                expectationFailure
                  ( "expected array < list; list="
                      ++ show listBytes
                      ++ " arr="
                      ++ show arrBytes
                  )
