...
import Fory.MetaString.Hash qualified as MSH
...
tests :: Spec
tests =
  describe "Fory.MetaString.Encoder vs pyfory" $
    sequence_
      [ it "namespace 'example' (LOWER_SPECIAL, length 5)" $
          check
            MSE.namespaceSpecialChars
            "example"
            MSE.LowerSpecial
            "12e063d640"
            0xea8563ec8fce1001
  ...
  where
    ...
    pad16 s
      | length s < 16 = replicate (16 - length s) '0' ++ s
      | otherwise = s
