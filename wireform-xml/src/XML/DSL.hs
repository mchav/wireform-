...
module XML.DSL
  ...
  , (/>), (//>), (|>)
  -- * Axes
  , child, anyChild, descendant, anyDescendant
  , parent, self, followingSibling, precedingSibling
  -- * Content
  , textContent, attribute, hasAttribute
  , textNodes, commentNodes
  -- * Filtering
  , where_, whereAttr, whereText, whereContains
  -- * Indexing
  , index, first, last
  -- * Running queries
  , select, selectOne, selectText
  -- * Aggregation
  , count
  -- * User extension points
  , liftQuery, liftPure, liftMaybe, liftFilter
  ) where
...
whereContains attrName substr = where_ (\n ->
