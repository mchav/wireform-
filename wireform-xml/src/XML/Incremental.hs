                 ...
                 | b == 0x3C ->
                     if i + 1 >= len
                       then (safe, SText, i)
                         ...
                         let !b2 = BSU.unsafeIndex bs (i + 1)
                         in case b2 of
                              0x21 ->
                                if i + 3 >= len
                                  then (safe, SText, i)
                                  else
                                    if BSU.unsafeIndex bs (i + 2) == 0x2D
                                      && BSU.unsafeIndex bs (i + 3) == 0x2D
                                      then go (i + 4) SComment safe
                                      else
                                        if BSU.unsafeIndex bs (i + 2) == 0x5B
                 ...
                 | b == 0x2D ->
                     if i + 2 >= len
                       then (safe, SComment, i)
                         ...
                         if BSU.unsafeIndex bs (i + 1) == 0x2D
                 ...
                 | b == 0x5D ->
                     if i + 2 >= len
                       then (safe, SCDATA, i)
                         ...
                         if BSU.unsafeIndex bs (i + 1) == 0x5D
                           && BSU.unsafeIndex bs (i + 2) == 0x3E
                           then go (i + 3) SText (i + 3)
                 ...
                 | b == 0x3F ->
                     if i + 1 >= len
                       then (safe, SPI, i)
                         ...
                         if BSU.unsafeIndex bs (i + 1) == 0x3E
...
withConcurrentParse
  ...
  -- ^ Queue size (e.g. 256)
  -> (SAXEvent -> IO ())
  -- ^ Event handler (runs in caller's thread)
  -> IO (Either String ())
