...
import XML.Encoding qualified as Enc
import XML.Value
...
instance (ToXML v) => ToXML (Map Text v) where
  toXML m =
    Element
      ...
      V.empty
      ( V.fromList
          [ Element (simpleName k) V.empty (V.singleton (toXML v))
          | (k, v) <- Map.toList m
          ]
      )
...
instance ToXML v => ToXML (IntMap v) where
  toXML m =
    Element
      ...
      V.empty
      ( V.fromList
          [ Element (simpleName (intToText k)) V.empty (V.singleton (toXML v))
          | (k, v) <- IntMap.toList m
          ]
      )
...
instance (ToXML a, ToXML b) => ToXML (a, b) where
  toXML (a, b) =
    Element
      ...
      V.empty
      ( V.fromList
          [ Element (simpleName "_1") V.empty (V.singleton (toXML a))
...
instance (Integral a, ToXML a) => ToXML (Ratio a) where
  toXML r =
    Element
      ...
      V.empty
      ( V.fromList
          [ Element (simpleName "num") V.empty (V.singleton (toXML (numerator r)))
...
instance (ToXML (f a), ToXML (g a)) => ToXML (FProduct.Product f g a) where
  toXML (FProduct.Pair x y) =
    Element
      (simpleName "tuple")
      V.empty
      ( V.fromList
          [ Element (simpleName "_1") V.empty (V.singleton (toXML x))
...
instance (GToXMLFields f) => GToXMLCon (M1 C c f) where
  gToXMLCon typeName (M1 x) =
    let fields = gToXMLFields x
        children =
          V.fromList
            [ Element (simpleName (T.pack k)) V.empty (V.singleton v)
            | (k, v) <- fields
            ]
