...
fromXMLRecord typeNm c = do
  ...
  lamE
    ...
    ( caseE
        (varE v)
        [ match
            (conP 'XV.Element [wildP, varP attrsVar, varP childrenVar])
            (normalB (pure bodyE))
            []
        , match
            wildP
  ...
  if miSkip mi
    then case miDefaults mi of
      ...
      Nothing ->
        [|
          Left
            ( "XML.Derive: missing 'defaults' for skipped field "
        ...
        _ ->
          [|
            case lookupChildElement $(pure keyExp) $(varE childrenVar) of
              Nothing ->
                Left
                  ( "XML.Derive: missing child element "
                      ++ T.unpack $(pure keyExp)
                  )
