...
writeMode outDir = do
  ...
  writeSample
    ...
    Schema
      { arrowFields =
          V.singleton $
            pField "ree" True ARunEndEncoded $
              V.fromList
  ...
  let zstdOpts = defaultWriteOptions {writeBodyCompression = Just BodyZstd}
      zstdSchema =
        Schema
          (V.singleton (pField "n" False (AInt 64 True) V.empty))
          Little
          V.empty
          V.empty
      zstdBatch = V.singleton (ColInt64 (VP.fromList ([1 .. 500] :: [Int64])))
  BS.writeFile
    (outDir <> "/ours_zstd_compressed.arrows")
  ...
  writeSample
    "float"
    ( Schema
        (V.singleton (pField "x" False (AFloatingPoint Single) V.empty))
        Little
        V.empty
        V.empty
    )
  ...
  writeSample
    "double"
    ( Schema
        (V.singleton (pField "x" False (AFloatingPoint DoublePrecision) V.empty))
        Little
        V.empty
        V.empty
    )
  ...
  writeSample
    "binary"
    ( Schema
        (V.singleton (pField "b" False ABinary V.empty))
        Little
        V.empty
        V.empty
    )
  ...
  writeSample
    "fixedbin16"
    ( Schema
        (V.singleton (pField "u" False (AFixedSizeBinary 16) V.empty))
        Little
        V.empty
        V.empty
    )
  ...
  writeSample
    "date32"
    ( Schema
        (V.singleton (pField "d" False (ADate DateDay) V.empty))
        Little
        V.empty
        V.empty
    )
  ...
  writeSample
    "time64_us"
    ( Schema
        (V.singleton (pField "t" False (ATime Microsecond 64) V.empty))
        Little
        V.empty
        V.empty
    )
    ...
    ( Schema
        ( V.singleton
            ( pField
                "ts"
                False
                (ATimestamp Nanosecond (Just "UTC"))
                V.empty
            )
        )
        Little
  ...
  writeSample
    "duration_ns"
    ( Schema
        (V.singleton (pField "d" False (ADuration Nanosecond) V.empty))
        Little
        V.empty
        V.empty
    )
  ...
  writeSample
    "decimal128"
    ( Schema
        (V.singleton (pField "d" False (ADecimal 18 2) V.empty))
        Little
        V.empty
        V.empty
    )
  ...
  writeSample
    ...
    ( Schema
        ( V.singleton $
            pField "m" False (AMap False) $
              V.singleton $
                pField "entries" False AStruct $
                  V.fromList
  ...
  writeSample
    "large_utf8"
    ( Schema
        (V.singleton (pField "s" False ALargeUtf8 V.empty))
        Little
        V.empty
        V.empty
    )
  ...
  writeSample
    "dense_union_int32_utf8"
    ( Schema
        ( V.singleton $
            pField "u" False (AUnion Dense (V.fromList [0, 1])) $
              V.fromList
                [ pField "i" False (AInt 32 True) V.empty
                , pField "s" False AUtf8 V.empty
                ]
