...
tests :: Spec
tests = describe "PerMessageDeflate" $ sequence_
  [ describe "offer parsing" $ sequence_
  ...
  , describe "selectOffer" $ sequence_
  ...
  , describe "response header" $ sequence_
  ...
  , describe "compress / decompress round-trip" $ sequence_
      [ it "ascii small" $ roundTrip defaultPmdParams
          "the quick brown fox jumps over the lazy dog"
      , it "highly compressible repetition" $ roundTrip defaultPmdParams
          (BS.replicate 16384 (fromIntegral (fromEnum 'A')))
      , it "incompressible (random-ish)" $ roundTrip defaultPmdParams $
      ...
      , it "many small messages with no_context_takeover" $
          manyMessages defaultPmdParams
            ...
            50
      , it "smaller window bits still round-trips" $ roundTrip
