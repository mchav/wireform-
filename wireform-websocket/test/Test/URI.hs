...
parseHappyPath :: Spec
parseHappyPath = describe "parses canonical URIs" $ sequence_
  [ it "ws with port and path" $
      parseWebSocketURI "ws://example.com:8080/chat?room=42"
        `shouldBe` Right WebSocketURI
              { wsuScheme = WsScheme
              , wsuHost   = "example.com"
              , wsuPort   = 8080
...
ipv6Literals :: Spec
ipv6Literals = describe "IPv6 literals" $ sequence_
  [ it "no port" $
...
badCases :: Spec
badCases = describe "rejects malformed URIs" $ sequence_
...
roundTrip :: Spec
roundTrip = describe "parse . render = id" $ sequence_
  [ it "ws default port" $
