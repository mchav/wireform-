...
buildTlsCtx t = do
  let cfg =
        (TLSCfg.defaultTlsServerConfig (wstCertPath t) (wstKeyPath t))
...
augmentResponseForPmd (Just p) r =
  r
    { responseHeaders =
        responseHeaders r
...
runHandlerOverDuplex
  ...
  -> Maybe TLS.SslConn
  -> ByteString
  {- ^ leftover bytes pulled past the header
  block; must be the first thing the
  parser sees.
  -}
  -> Maybe PMD.PmdParams
  {- ^ negotiated PMD parameters, or
