...
import Foreign.Marshal.Alloc (alloca, mallocBytes, reallocBytes, free)
...
parseOffers raw =
  ...
  where
    ...
    stripQuotes b
      | BS.length b >= 2
      , BS.head b == 0x22
      , BS.last b == 0x22 = BS.init (BS.tail b)
      | otherwise         = b
...
selectOffer
  :: PmdParams        -- ^ server's preferred ceiling
  -> [PmdOffer]
  -> Maybe PmdParams
selectOffer policy = foldr step Nothing
  where
    step off acc =
      let serverWb = min (pmdServerMaxWindowBits policy)
          ...
          serverNc = offServerNoContextTakeover off
                  || pmdServerNoContextTakeover policy
          clientNc = offClientNoContextTakeover off
                  || pmdClientNoContextTakeover policy
          ok = serverWb >= 8 && serverWb <= 15
            && clientWb >= 8 && clientWb <= 15
      in if ok
           then Just PmdParams
...
responseHeader p = BS.intercalate "; " $
  "permessage-deflate"
  : concat
...
newPmdContext role params = do
  ...
  defPtr <- c_wf_pmd_deflate_new (fromIntegral defWb)
...
driveZ s input op _what =
  ...
  where
    ...
    drive srcPtr srcLen buf produced cap = alloca $ \outProducedRef -> do
      ...
      rc <- op s srcPtr (fromIntegral srcLen)
                writePtr (fromIntegral remaining)
                outProducedRef
